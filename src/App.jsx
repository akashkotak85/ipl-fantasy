...
/* --- INLINE REVEAL STRIP Ã¢ÂÂ auto shown below locked match cards --- */
  ...
  return(
    <div style={{borderTop:"1px solid #f1f5f9",paddingTop:10,marginTop:4}}>
      <div style={{marginBottom:8}}>
        <span style={{fontSize:10,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5}}>Ã°ÂÂÂ¥ Group Picks</span>
      </div>
          ...
          return(
            <div key={u.email} style={{background:cardBg,border:"1px solid "+borderCol,borderRadius:10,padding:"8px 10px",display:"flex",alignItems:"center",gap:8}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontSize:11,fontWeight:700,color:"#1a2540",margin:0}}>{u.name}{perfect&&" Ã°ÂÂÂ"}</p>
                <div style={{display:"flex",gap:4,marginTop:3,flexWrap:"wrap"}}>
                  {[
                    ...
                    <span key={lbl} style={{fontSize:9,padding:"1px 5px",borderRadius:4,fontWeight:700,
                      ...
                      {lbl}: {val||"Ã¢ÂÂ"}{hasResult&&avail?(ok?" Ã¢ÂÂ":" Ã¢ÂÂ"):""}
                    </span>
                  ))}
                  {bandShort&&<span style={{fontSize:9,padding:"1px 5px",borderRadius:4,fontWeight:700,
                    ...
                    Ã°ÂÂÂ {bandShort}{sbAns?(sbOk?" Ã¢ÂÂ":" Ã¢ÂÂ"):""}
                  </span>}
                  {(()=>{const bq=(allBonusPicks?.[ek(u.email)]||{})[String(m.id)];if(bq==null)return null;return<span style={{fontSize:9,padding:"1px 5px",borderRadius:4,fontWeight:700,background:bonusAns!=null?(bqOk?"#dcfce7":"#fee2e2"):"#f1f5f9",color:bonusAns!=null?(bqOk?"#15803d":"#dc2626"):"#94a3b8"}}>Ã°ÂÂÂ{bq?"Y":"N"}{bonusAns!=null?(bqOk?" Ã¢ÂÂ":" Ã¢ÂÂ"):""}
                  </span>;})()}
                </div>
              </div>
            </div>
...
/* --- MATCH CARD --- */
function MCard({m,pred,myPicks,allPicks,rxns,doubleMatch,lockedMatches,matchPtsOverride,email,allMs,onPredict,onReact,bonusAnswers,myBonusPicks,allBonusPicks,scoreBandAnswers,onBonusPick,users,TC,TF,BONUS_QUESTIONS}){
  ...
  return(
    <div className="mcard fade-in">
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <span style={{color:"#64748b",fontSize:11,fontWeight:600}}>{m.mn} ÃÂ· {m.date} ÃÂ· {m.time}</span>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {isDouble&&<span style={{background:"linear-gradient(135deg,#FF822A,#D4AF37)",color:"#fff",fontSize:9,padding:"2px 7px",borderRadius:20,fontWeight:800}}>Ã¢ÂÂ¡ 2ÃÂ</span>}
          {isWashout&&<span style={{background:"#f1f5f9",color:"#64748b",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Ã°ÂÂÂ§Ã¯Â¸Â Washout</span>}
          {m.result&&!isWashout
            ?<span style={{background:"#dbeafe",color:"#1e40af",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Done</span>
            :!m.result&&lk
            ?<span style={{background:"#fee2e2",color:"#991b1b",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Ã°ÂÂÂ Locked</span>
            :!m.result
            ?<span style={{background:"#dcfce7",color:"#166534",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Ã°ÂÂÂ¢ Open</span>
            :null}
        </div>
      </div>
      ...
      {hasForm&&(
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:8}}>
          ...
          <span style={{color:"#94a3b8",fontSize:9}}>ÃÂ· last {Math.max(homeForm.length,awayForm.length)} (oldest?newest)</span>
        </div>
      )}

      <p style={{color:"#94a3b8",fontSize:11,borderTop:"1px solid #f1f5f9",paddingTop:8,marginBottom:10}}>Ã°ÂÂÂ {m.venue}</p>
      ...
      {m.result&&(
        <div style={{background:"#F4F6FB",borderRadius:8,padding:"8px 12px",fontSize:12,marginBottom:8}}>
          ...
          <span style={{color:"#94a3b8",margin:"0 6px"}}>ÃÂ·</span>
      ...
      {!m.result&&m._partial&&(
        <div style={{background:"linear-gradient(135deg,#FFF9E6,#FEF3C7)",border:"1px solid #FDE68A",borderRadius:12,padding:"12px 14px",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
            <span style={{fontSize:13}}>Ã°ÂÂÂ´</span>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {[
              ["Ã°ÂÂªÂ","Toss",m._partial.toss,mp?.toss],
              ["Ã°ÂÂÂ","Winner",m._partial.win,mp?.win],
              ["Ã¢Â­Â","POTM",m._partial.motm?.split(" ").slice(-1)[0],mp?.motm?.split(" ").slice(-1)[0]],
                ...
                <div key={lbl} style={{background:"rgba(255,255,255,.8)",borderRadius:8,padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  ...
                  {myv&&<div style={{textAlign:"right"}}>
                    ...
                    <p style={{fontSize:12,fontWeight:700,margin:0,color:isRight?"#15803d":isWrong?"#dc2626":"#1a2540"}}>
                      {myv} {isRight?"Ã¢ÂÂ":isWrong?"Ã¢ÂÂ":""}
                    </p>
                  </div>}
                </div>
              );
          ...
          <p style={{fontSize:10,color:"#B45309",margin:"8px 0 0",textAlign:"center"}}>More results comingÃ¢ÂÂ¦ being finalised by admin</p>
        </div>
      )}
      ...
      {(()=>{
        ...
        return<div style={{background:"#F4F6FB",border:"1px solid #e2e8f0",borderRadius:8,padding:"7px 12px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:6}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:11,color:"#64748b"}}>1st Innings: <b style={{color:"#1a2540"}}>{bandObj?.label||"Ã¢ÂÂ"}</b></span>
          </div>
          {sbAns&&<span style={{fontSize:11,fontWeight:700,color:isCorrect?"#15803d":isWrong?"#dc2626":"#94a3b8"}}>
            {isCorrect?`Ã¢ÂÂ Correct! +${PTS.scoreBand}pts`:isWrong?`Ã¢ÂÂ Was: ${correctBandObj?.short}`:correctBandObj?`Correct: ${correctBandObj?.short}`:"Result TBD"}
          </span>}
      ...
      {hints&&(
        <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
          <p style={{color:"#92400E",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:"0 0 8px"}}>
            Ã°ÂÂÂ Group Leans ÃÂ· {hints.tot} pick{hints.tot!==1?"s":""}
          </p>
      ...
      {/* -- BONUS QUESTION (read-only Ã¢ÂÂ answered in prediction screen) -- */}
      {!isTBD(m)&&BONUS_QUESTIONS[m.id]&&(()=>{
        ...
        return<div style={{background:"#F4F6FB",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
            <span style={{fontSize:12}}>Ã°ÂÂÂ</span>
            <span style={{fontSize:10,fontWeight:700,color:"#1D428A",textTransform:"uppercase",letterSpacing:.5}}>Bonus Question ÃÂ· +{PTS.bonus}pts</span>
          ...
          {myAns!=null
            ?<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                <span className={"bq-btn "+(myAns?"yes":"no")+" on"} style={{display:"inline-block",textAlign:"center",padding:"6px 14px",fontSize:12}}>{myAns?"Ã¢ÂÂ Yes":"Ã¢ÂÂ No"}</span>
                {bonusAns2!=null&&<span style={{fontSize:11,color:bqPts>0?"#15803d":"#dc2626",fontWeight:700}}>{bonusAns2?"Correct: Yes":"Correct: No"}{bqPts!==null?" ÃÂ· "+(bqPts>0?"Ã¢ÂÂ +"+bqPts+"pts":"Ã¢ÂÂ 0pts"):""}</span>}
              </div>
          ...
          {tot2>0&&lk&&<p style={{fontSize:10,color:"#94a3b8",margin:"6px 0 0"}}>Group: {yesCount} Yes ÃÂ· {noCount} No</p>}
        </div>;
      })()}

      {/* -- AUTO INLINE REVEAL Ã¢ÂÂ shows below card when locked -- */}
      ...
      {pred&&lk&&!mp&&!m.result&&<div style={{textAlign:"center",padding:"8px",fontSize:12,color:"#991b1b",marginTop:4}}>Ã°ÂÂÂ Prediction window closed</div>}
    </div>
  );
}
...
function AdminManualPickPanel({ms,users,allPicks,doubleMatch,onSave,onSaveSeasonData,spk,t4pk,allPropBets,allBonusPicks,toast2,TEAMS,TF,BONUS_QUESTIONS,ALL_PLAYERS,PROP_QUESTIONS}){
  ...
  async function handleSave(){
    ...
    if(!draft.toss||!draft.win||!draft.motm||!draft.sb){toast2("Fill Q1Ã¢ÂÂQ4","error");return;}
  ...
  return(
    <div>
    <div className="ac">
      <p className="st" style={{marginBottom:12}}>Ã°ÂÂÂ¸ MANUAL PICK ENTRY (SCREENSHOT EVIDENCE)</p>
      <div style={{background:"#FFF9E6",border:"1px solid #FDE68A",borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:12,color:"#92400E"}}>
        Ã¢ÂÂ Ã¯Â¸Â Use only when a player sent screenshot proof of their prediction before lock time. This bypasses the lock Ã¢ÂÂ use responsibly.
      </div>

      {/* Step 1: User */}
      <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>Step 1 Ã¢ÂÂ Select Player</p>
      <input className="inp" placeholder="Search playerÃ¢ÂÂ¦" value={userSearch} onChange={e=>setUserSearch(e.target.value)} style={{marginBottom:8}}/>
          ...
          return(
            <div key={u.email} onClick={()=>selectUser(u.email)}
              ...
              <div style={{flex:1}}>
                ...
                <p style={{fontSize:10,color:"#94a3b8",margin:0}}>{u.email} ÃÂ· {pickCount} picks</p>
              </div>
              {selUser===u.email&&<span style={{color:"#1D428A",fontSize:14}}>Ã¢ÂÂ</span>}
            </div>
          );
        })}
      </div>
      ...
      {/* Step 2: Match */}
      {selUser&&<>
        <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>Step 2 Ã¢ÂÂ Select Match</p>
        <input className="inp" placeholder="Search match (M17, PBKS, CSKÃ¢ÂÂ¦)" value={matchSearch} onChange={e=>setMatchSearch(e.target.value)} style={{marginBottom:8}}/>
            ...
            return(
              <div key={m.id} onClick={()=>{setSelMatch(m);setDraft({toss:"",win:"",motm:"",sb:"",bqAns:null});setMatchSearch("");}}
                ...
                <div style={{flex:1}}>
                  ...
                  <p style={{fontSize:10,color:"#94a3b8",margin:0}}>{m.date} ÃÂ· {m.time}</p>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:3}}>
                  ...
                  {locked&&<span style={{background:"#fee2e2",color:"#991b1b",fontSize:9,padding:"2px 6px",borderRadius:8,fontWeight:700}}>Ã°ÂÂÂ</span>}
                  {m.result&&<span style={{background:"#dbeafe",color:"#1e40af",fontSize:9,padding:"2px 6px",borderRadius:8,fontWeight:700}}>Done</span>}
                  {selMatch?.id===m.id&&<span style={{color:"#1D428A",fontSize:14}}>Ã¢ÂÂ</span>}
                </div>
              </div>
            );
          })}
      ...
      {existingPick&&<div style={{background:"#FEF3C7",border:"1px solid #FDE68A",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:12,color:"#92400E"}}>
        Ã¢ÂÂ¹Ã¯Â¸Â Existing pick for {selMatch?.mn}: <b>{existingPick.toss}</b> toss ÃÂ· <b>{existingPick.win}</b> win ÃÂ· POTM: <b>{existingPick.motm?.split(" ").slice(-1)[0]}</b>{existingPick.sb?` ÃÂ· Band: ${existingPick.sb}`:""}. Saving will overwrite.
        {existingBQ!==undefined&&<span> ÃÂ· Bonus: {existingBQ?"Yes":"No"}</span>}
      </div>}
      ...
      {selUser&&selMatch&&<>
        <div style={{background:"#EBF0FA",border:"1px solid #bfdbfe",borderRadius:12,padding:"14px",marginBottom:14}}>
          <p style={{fontSize:11,fontWeight:700,color:"#1e40af",textTransform:"uppercase",letterSpacing:.5,marginBottom:14}}>
            Step 3 Ã¢ÂÂ Enter All 5 Picks ÃÂ· {selMatch.mn}: {selMatch.home} vs {selMatch.away}
          </p>

          {/* Q1: Toss */}
          <p style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:6}}>Q1 ÃÂ· Toss Winner</p>
          ...
          {/* Q2: Winner */}
          <p style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:6}}>Q2 ÃÂ· Match Winner</p>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {[selMatch.home,selMatch.away].map(t=>(
          ...
          {/* Q3: POTM */}
          <p style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:6}}>Q3 ÃÂ· Player of the Match</p>
          <div style={{marginBottom:14}}>
            <PotmDropdown homeTeam={selMatch.home} awayTeam={selMatch.away} value={draft.motm} onChange={v=>setDraft(d=>({...d,motm:v}))}/>
          </div>

          {/* Q4: Score Band */}
          <p style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:6}}>Q4 ÃÂ· First Innings Score Band</p>
          ...
          {!draft.sb&&<p style={{fontSize:10,color:"#ef4444",marginBottom:10,fontWeight:600}}>Ã¢ÂÂ Ã¯Â¸Â Required</p>}
          {draft.sb&&<p style={{fontSize:10,color:"#94a3b8",marginBottom:10}}>Selected: {SCORE_BANDS.find(b=>b.id===draft.sb)?.label}</p>}

          {/* Q5: Bonus Question */}
          {hasBQ&&<>
            <p style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:4}}>Q5 ÃÂ· Bonus Question</p>
            ...
            <div style={{display:"flex",gap:8,marginBottom:draft.bqAns===null?4:10}}>
              <button className={"bq-btn yes"+(draft.bqAns===true?" on":"")} onClick={()=>setDraft(d=>({...d,bqAns:d.bqAns===true?null:true}))}>Ã¢ÂÂ Yes</button>
              <button className={"bq-btn no"+(draft.bqAns===false?" on":"")} onClick={()=>setDraft(d=>({...d,bqAns:d.bqAns===false?null:false}))}>Ã¢ÂÂ No</button>
            </div>
            {draft.bqAns===null&&<p style={{fontSize:10,color:"#ef4444",marginBottom:10,fontWeight:600}}>Ã¢ÂÂ Ã¯Â¸Â Required</p>}
          </>}
          ...
          {allReady&&<div style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginTop:4,border:"1px solid #bfdbfe",fontSize:12}}>
            ...
            <p style={{color:"#1a2540",margin:0,lineHeight:1.6}}>
              <b>{Object.values(users).find(u=>u.email===selUser)?.name||selUser}</b> Ã¢ÂÂ {selMatch.mn}:{" "}
              <b>{draft.toss}</b> toss ÃÂ· <b>{draft.win}</b> win ÃÂ· <b>{draft.motm?.split(" ").slice(-1)[0]}</b> POTM ÃÂ· <b>{draft.sb}</b> band{hasBQ?` ÃÂ· Bonus: ${draft.bqAns?"Yes":"No"}`:""}
            </p>
          </div>}
        </div>

        <button className="pbtn" disabled={saving||!allReady} onClick={handleSave} style={{marginBottom:8}}>
          {saving?"SavingÃ¢ÂÂ¦":"Ã°ÂÂÂ¾ Save Pick for "+Object.values(users).find(u=>u.email===selUser)?.name}
        </button>
      </>}
    </div>
    ...
    {selUser&&<div className="ac" style={{marginTop:14}}>
      <p className="st" style={{marginBottom:4}}>Ã¢ÂÂÃ¯Â¸Â SEASON DATA OVERRIDE</p>
      ...
      {/* Champion */}
      <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>Ã°ÂÂÂ Champion Pick</p>
      ...
      <button className="pbtn" disabled={!ovChamp||savingSeason==="champ"} onClick={handleSaveChampion} style={{marginBottom:14,opacity:ovChamp?1:.4}}>
        {savingSeason==="champ"?"SavingÃ¢ÂÂ¦":"Ã°ÂÂÂ¾ Save Champion: "+(ovChamp||"Ã¢ÂÂ")}
      </button>

      {/* Top 4 */}
      <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>Ã°ÂÂÂ Top 4 Picks ÃÂ· {ovT4.length}/4</p>
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>
      ...
      <button className="pbtn" disabled={ovT4.length!==4||savingSeason==="t4"} onClick={handleSaveTop4} style={{marginBottom:14,opacity:ovT4.length===4?1:.4}}>
        {savingSeason==="t4"?"SavingÃ¢ÂÂ¦":"Ã°ÂÂÂ¾ Save Top 4"}
      </button>

      {/* Prop Bets */}
      <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>Ã°ÂÂÂ² Season Prop Bets</p>
      {PROP_QUESTIONS.map((q,i)=>{
        const val=ovProps[q.id]||"";
        return<div key={q.id} style={{marginBottom:10}}>
          <p style={{fontSize:11,fontWeight:700,color:"#1D428A",margin:"0 0 4px"}}>Q{i+1} ÃÂ· {q.label}</p>
          {q.type==="player"&&<select className="sel" value={val} onChange={e=>setOvProps(p=>({...p,[q.id]:e.target.value}))}><option value="">Select playerÃ¢ÂÂ¦</option>{ALL_PLAYERS.map(({p,t})=><option key={p+t} value={p}>{p} ({t})</option>)}</select>}
          {q.type==="team"&&<select className="sel" value={val} onChange={e=>setOvProps(p=>({...p,[q.id]:e.target.value}))}><option value="">Select teamÃ¢ÂÂ¦</option>{TEAMS.map(t=><option key={t} value={t}>{TF[t]}</option>)}</select>}
          {q.type==="yesno"&&<div style={{display:"flex",gap:8}}><button className={"bq-btn yes"+(val==="true"?" on":"")} onClick={()=>setOvProps(p=>({...p,[q.id]:"true"}))}>Ã¢ÂÂ Yes</button><button className={"bq-btn no"+(val==="false"?" on":"")} onClick={()=>setOvProps(p=>({...p,[q.id]:"false"}))}>Ã¢ÂÂ No</button></div>}
        </div>;
      })}
      <button className="pbtn" disabled={savingSeason==="props"} onClick={handleSaveProps} style={{background:"linear-gradient(135deg,#0f6e56,#1D9E75)"}}>
        {savingSeason==="props"?"SavingÃ¢ÂÂ¦":"Ã°ÂÂÂ¾ Save Prop Bets"}
      </button>
    </div>}
    </div>
  );
...
/* ----------------------------------------------------------------
   NAV Ã¢ÂÂ defined outside App so React never remounts it on re-renders
   ---------------------------------------------------------------- */
...
// Tournament-specific data Ã¢ÂÂ falls back to IPL 2026 defaults when no tournament selected
...
setActiveVisuals({TC:selectedTournament?.TC,SQ:selectedTournament?.SQ,LOGOS:selectedTournament?.LOGOS});
...
const[actualTop4,setActualTop4]=useState([]);
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{const pu=await DB.get("pending")||{};const cnt=Object.keys(pu).length;setPendingUsers(pu);if(cnt>lastPendingCount.current)toast2("Ã°ÂÂÂ New registration pending approval","info");lastPendingCount.current=cnt;};
    poll();const id=setInterval(poll,20000);return()=>clearInterval(id);
  },[isAdmin,user]);// eslint-disable-line

  /*
    FIX Ã¢ÂÂ forceRepair: now calls DB.repairAllPicks which re-normalises ALL
  ...
  */
  ...
  const reloadShared=useCallback(async(em)=>{
    ...
    // FIX: always normalise the ap map on every reload Ã¢ÂÂ this converts any
    ...
    setManualPtsAdj(pts?normalizeKeyMap(pts):{});
    setLockedMatches(lk||{});
    setPinnedBc(pbc||null);
    setDoubleMatch(dm!=null?dm:null);
    if(cm2!=null)setChatMuted(!!cm2);
    if(mu)setMutedUsers(normalizeKeyMap(mu));
    setMatchPtsOverride(mpo?normalizeKeyMap(mpo):{});
    setBonusAnswers(bonusAns||{});
    setScoreBandAnswers(sba||{});
    ...
    setPropAnswers(paAll||{});
    setActualTop4(at4&&Array.isArray(at4)?at4:[]);
...
try{localStorage.removeItem("ipl26_session");}catch(e){}if(!cancelled)setSc("login");return;}
        ...
        // FIX: repair DB on every auto-login REMOVED Ã¢ÂÂ too dangerous (caused data loss).
  ...
  async function doSignIn(em,ex,isNew=false){
    ...
    if(isNew||!hasOnboarded)setSc("onboard");
      ...
      setObProps({q0:userPropBets.q0||"",q1:userPropBets.q1||"",q2:userPropBets.q2||"",q3:userPropBets.q3||"",q4:userPropBets.q4||""});
      setSc("propbets");toast2("One quick thing Ã¢ÂÂ fill in your season prop bets! Ã°ÂÂÂ");
    }
    else{setSc("sport_select");toast2("Welcome back, "+ex.name+"! Ã°ÂÂÂ","ok");}
  }
  ...
  async function approveUser(emk){const pu=await DB.get("pending")||{};const entry=pu[emk];if(!entry)return;delete pu[emk];const u2=await DB.get("u")||{};u2[entry.email]={...entry,approved:true};await DB.set("u",u2);await DB.set("pending",pu);setUsers({...users,[entry.email]:{...entry,approved:true}});setPendingUsers({...pu});toast2(entry.name+" approved! Ã¢ÂÂ","ok");const latest=await DB.get("ch")||[];const nc=capChat([...latest,{id:Date.now(),email:"__sys__",name:"Fantasy Bot",text:"Ã°ÂÂÂ "+entry.name+" has joined! Welcome!",ts:Date.now(),sys:true}]);setChat(nc);await DB.set("ch",nc);}
  ...
  async function doneOnboard(){
    ...
    if(unanswered.length>0){toast2("Answer all "+unanswered.length+" of "+PROP_QUESTIONS.length+" prop bet questions","error");return;}
    ...
    setSc("home");toast2("All picks locked! Let the games begin! Ã°ÂÂÂ","ok");
  }

  /*
    FIX Ã¢ÂÂ submitPick (complete rewrite):
    ----------------------------------------------------------------------------
    ...
    potentially coerced keys Ã¢ÂÂ normalizeAP dropped them Ã¢ÂÂ picks vanished.
    ...
    5. Only THEN navigate away Ã¢ÂÂ UI reflects what Firebase actually stored
  ...
  */
  async function submitPick(){
    if(!am)return;
    if(getP(myPicks,am.id)){toast2("Prediction already locked Ã¢ÂÂ no edits allowed","error");return;}
    ...
    const sid=String(am.id); // Always string Ã¢ÂÂ never numeric
    ...
    toast2("Prediction locked! Ã°ÂÂÂ","ok");
    setAm(null);
    setSc("home");
  }
  ...
  async function setManualResult(mid,overrideResult){
    ...
    const sbLine=sbAnsSet?`\nÃ°ÂÂÂ Score Band: ${SCORE_BANDS.find(b=>b.id===sbAnsSet)?.label||sbAnsSet} (+${PTS.scoreBand}pts for correct)`:"";
    const bonusLine=bonusQ&&bonusAnsSet!=null?`\nÃ°ÂÂÂ Bonus: ${bonusAnsSet?"YES":"NO"} (+${PTS.bonus}pts for correct)`:"";
    const newCh=capChat([...latest,{id:Date.now(),email:"__sys__",name:"Fantasy Bot",text:trashMsg+sbLine+bonusLine,ts:Date.now(),sys:true}]);
    setChat(newCh);await DB.set("ch",newCh);
    toast2("Result saved! Ã¢ÂÂ","ok");
    await reloadShared(email);
  }

  // Reset a single user's pick + bonus for a match, then refresh state in place
  // (no full page reload Ã¢ÂÂ keeps the admin inside the current tournament).
  ...
  /*
    Admin manual pick save Ã¢ÂÂ called by AdminManualPickPanel.
  ...
  */
  async function adminSavePick(targetEmail,match,pick,bqAns){
    ...
    toast2("Ã¢ÂÂ Pick saved for "+targetName,"ok");
    return true;
  }
  ...
  async function adminSaveSeasonData(targetEmail,type,data){
    ...
    if(type==="champion"){
      ...
      toast2("Ã¢ÂÂ Prop bets saved for "+targetName,"ok");
    }
  }

  /*
    Admin repair all picks Ã¢ÂÂ re-normalises every key in the ap subtree.
  ...
  */
  ...
  async function adminRepairDB(){
    ...
    try{
      await forceRepair();
      await reloadShared(email);
      toast2("Ã¢ÂÂ DB repaired & reloaded","ok");
    }catch(e){
      console.error("adminRepairDB",e);
      toast2("Repair failed Ã¢ÂÂ check console","error");
    }
  ...
  async function undoResult(mid){
    ...
    setAdmResultForm(prev=>{const n={...prev};delete n[mid];return n;});
    toast2("Result cleared Ã¢ÂÂ re-enter below","ok");
  }
  ...
  async function sendBc(pin=false){if(!bcMsg.trim())return;const nb=[...bc,{id:Date.now(),msg:bcMsg.trim(),ts:Date.now(),type:"admin"}];setBc(nb);await DB.set("bc",nb);if(pin){setPinnedBc(bcMsg.trim());await DB.set("pinnedbc",bcMsg.trim());}setBcMsg("");toast2(pin?"Ã°ÂÂÂ Pinned!":"Sent!","ok");}
  ...
  async function setSeasonWinner(t){setSw(t);await DB.set("sw",t);toast2("Champion: "+t+" Ã°ÂÂÂ","ok");}
  async function toggleMaintenance(v){setMaintenance(v);await DB.set("maintenance",v);toast2(v?"Ã°ÂÂÂ App locked":"Ã°ÂÂÂ¢ App live","ok");}
  async function submitBonusPick(mid,ans){
    ...
    toast2(ans?"Bonus: Yes locked Ã°ÂÂÂ":"Bonus: No locked Ã°ÂÂÂ","ok");
  }
  ...
  async function toggleSport(sport,enabled){
    ...
    toast2((enabled?"Ã¢ÂÂ ":"Ã°ÂÂÂ« ")+sport.toUpperCase()+" "+(enabled?"enabled":"disabled"),"ok");
  }
  ...
  async function handleStockPayout(payoutPtsMap){
    ...
    toast2(`Ã°ÂÂÂ° Market payout applied to ${Object.keys(payoutPtsMap).length} players`,"ok");
  }
  ...
  async function toggleReminder(mid){const upd={...reminders,[mid]:!reminders[mid]};setReminders(upd);await DB.set("rms",upd);toast2(upd[mid]?"Ã°ÂÂÂ Reminder set":"Ã°ÂÂÂ Reminder off");}
  ...
  const cardProps={myPicks,allPicks,rxns,doubleMatch,lockedMatches,matchPtsOverride,email,allMs:ms,onReact:reactFn,
    ...
    onPredict:(m)=>{
      if(getP(myPicks,m.id)){toast2("Prediction already locked Ã¢ÂÂ no edits allowed","error");return;}
  ...
  const hdr=useMemo(()=><div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",padding:"13px 16px 11px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      <div><p className="C" style={{color:"#FFE57F",fontSize:13,fontWeight:700,letterSpacing:1,margin:0,textTransform:"uppercase"}}>Cricket Fantasy{isAdmin?" ÃÂ· Admin":""}</p><p style={{color:"#bfdbfe",fontSize:10,margin:0}}>{selectedTournament?.name||"IPL 2026"}{maintenance?" ÃÂ· !":""}</p></div>
    </div>
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      <button onClick={()=>setSc("sport_select")} style={{background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.2)",borderRadius:8,color:"#bfdbfe",fontSize:11,padding:"5px 8px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600}}>Ã¢ÂÂ½ Sports</button>
      <button onClick={()=>setShowCareer(true)} style={{background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.2)",borderRadius:8,color:"#bfdbfe",fontSize:11,padding:"5px 8px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600}}>Ã°ÂÂÂ Career</button>
  ...
  if(sc==="fifa"){
    return(
      <React.Suspense fallback={<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#64748b",fontSize:14}}>Loading...</div>}>
...
<button className="pbtn" disabled={authLoading} onClick={doLogin}>{authLoading?"Signing inÃ¢ÂÂ¦":"Sign In"}</button><p style={{color:"#94a3b8",fontSize:11,textAlign:"center"}}>No account? <button onClick={()=>{setAuthMode("register");clearAuthForm();}} style={{background:"none",border:"none",color:"#1D428A",fontSize:11,cursor:"pointer",fontWeight:600}}>Create one Ã¢ÂÂ</button></p></>}{authMode==="register"&&<><div style={{background:"#EBF0FA",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#1e40af"}}>Ã¢ÂÂ¹Ã¯Â¸Â New accounts require admin approval.</div><div><input className={"inp"+(authErrors.name?" err":"")} value={authName} onChange={e=>{setAuthName(e.target.value);setAuthErrors(p=>({...p,name:""}));}} placeholder="Full name"/>{authErrors.name&&<p className="ferr">{authErrors.name}</p>}</div><div><input className={"inp"+(authErrors.email?" err":"")} value={authEmail} onChange={e=>{setAuthEmail(e.target.value);setAuthErrors(p=>({...p,email:""}));}} placeholder="Email address" autoCapitalize="off" autoCorrect="off"/>{authErrors.email&&<p className="ferr">{authErrors.email}</p>}</div><div><div style={{position:"relative"}}><input className={"inp"+(authErrors.pw?" err":"")} type={showPw?"text":"password"} value={authPw} onChange={e=>{setAuthPw(e.target.value);setAuthErrors(p=>({...p,pw:""}));}} placeholder="Password" style={{paddingRight:48}}/><button onClick={()=>setShowPw(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showPw?"Ã°ÂÂÂ":"Ã°ÂÂÂÃ¯Â¸Â"}</button></div><p style={{color:"#94a3b8",fontSize:10,marginTop:4}}>Min 8 chars ÃÂ· uppercase ÃÂ· number ÃÂ· special char</p>{authErrors.pw&&<p className="ferr">{authErrors.pw}</p>}</div><div><div style={{position:"relative"}}><input className={"inp"+(authErrors.pw2?" err":"")} type={showPw2?"text":"password"} value={authPw2} onChange={e=>{setAuthPw2(e.target.value);setAuthErrors(p=>({...p,pw2:""}));}} placeholder="Confirm password" style={{paddingRight:48}} onKeyDown={e=>e.key==="Enter"&&doRegister()}/><button onClick={()=>setShowPw2(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showPw2?"hide":"show"}</button></div>{authErrors.pw2&&<p className="ferr">{authErrors.pw2}</p>}</div><button className="pbtn" disabled={authLoading} onClick={doRegister}>{authLoading?"SubmittingÃ¢ÂÂ¦":"Submit for Approval"}</button><p style={{color:"#94a3b8",fontSize:11,textAlign:"center"}}>Already registered? <button onClick={()=>{setAuthMode("login");clearAuthForm();}} style={{background:"none",border:"none",color:"#1D428A",fontSize:11,cursor:"pointer",fontWeight:600}}>Sign in Ã¢ÂÂ</button></p></>}{authMode==="forgot"&&<>{forgotStep===1&&<><div style={{background:"#EBF0FA",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#1e40af"}}>Enter your registered email to reset your password.</div><div><input className={"inp"+(authErrors.email?" err":"")} value={authEmail} onChange={e=>{setAuthEmail(e.target.value);setAuthErrors(p=>({...p,email:""}));}} placeholder="Email address" autoCapitalize="off"/>{authErrors.email&&<p className="ferr">{authErrors.email}</p>}</div><button className="pbtn" disabled={authLoading} onClick={doForgotStep1}>{authLoading?"CheckingÃ¢ÂÂ¦":"Verify Email"}</button></>}{forgotStep===2&&<><div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#166534"}}>Ã¢ÂÂ Verified: <b>{authEmail}</b></div><div><div style={{position:"relative"}}><input className={"inp"+(authErrors.pw?" err":"")} type={showForgotPw?"text":"password"} value={forgotNewPw} onChange={e=>{setForgotNewPw(e.target.value);setAuthErrors(p=>({...p,pw:""}));}} placeholder="New password" style={{paddingRight:48}}/><button onClick={()=>setShowForgotPw(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showForgotPw?"hide":"show"}</button></div>{authErrors.pw&&<p className="ferr">{authErrors.pw}</p>}</div><div><div style={{position:"relative"}}><input className={"inp"+(authErrors.pw2?" err":"")} type={showForgotPw2?"text":"password"} value={forgotNewPw2} onChange={e=>{setForgotNewPw2(e.target.value);setAuthErrors(p=>({...p,pw2:""}));}} placeholder="Confirm new password" style={{paddingRight:48}} onKeyDown={e=>e.key==="Enter"&&doForgotStep2()}/><button onClick={()=>setShowForgotPw2(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showForgotPw2?"hide":"show"}</button></div>{authErrors.pw2&&<p className="ferr">{authErrors.pw2}</p>}</div><button className="pbtn" disabled={authLoading} onClick={doForgotStep2}>{authLoading?"SavingÃ¢ÂÂ¦":"Set New Password"}</button><button onClick={()=>{setForgotStep(1);setForgotNewPw("");setForgotNewPw2("");setAuthErrors({});}} style={{background:"none",border:"none",color:"#94a3b8",fontSize:12,cursor:"pointer"}}>Ã¢ÂÂ Back</button></>}<button onClick={()=>{setAuthMode("login");clearAuthForm();}} style={{background:"none",border:"none",color:"#1D428A",fontSize:13,cursor:"pointer",fontWeight:600,marginTop:4}}>Ã¢ÂÂ Back to Sign In</button></>}</div>{toast&&<Tst t={toast}/>}</div>;
  ...
  if(sc==="onboard")return<div className="app" style={{minHeight:"100vh"}}><style>{CSS}</style>
    ...
    <div style={{padding:"20px 16px"}}>
      {obStep===0&&<>
        <p style={{color:"#1a2540",fontSize:15,fontWeight:600,margin:"0 0 6px"}}>Who will win {selectedTournament?.name||"IPL 2026"}?</p>
      ...
      {obStep===1&&<>
        ...
        <p style={{color:"#94a3b8",fontSize:12,margin:"0 0 14px"}}>Select exactly 4 teams ÃÂ· {obT4.length}/4</p>
        ...
        <button style={{width:"100%",padding:"12px",borderRadius:10,background:"#f1f5f9",color:"#475569",border:"1px solid #e2e8f0",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:15,textTransform:"uppercase",marginBottom:10}} onClick={()=>updateObStep(0,obSp,obT4)}>Ã¢ÂÂ Back</button>
      ...
      {obStep===2&&<>
        ...
        <p style={{color:"#64748b",fontSize:12,margin:"0 0 4px"}}>Each correct answer = <b style={{color:"#1D428A"}}>+{PTS.prop}pts</b>. Set once Ã¢ÂÂ locked for the season.</p>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:16}}>
          ...
          <span style={{fontSize:11,color:"#64748b",whiteSpace:"nowrap",fontWeight:600,marginLeft:4}}>
            {PROP_QUESTIONS.filter((q,i)=>obProps[`q${i}`]&&obProps[`q${i}`]!=="").length}/{PROP_QUESTIONS.length} answered
          </span>
        </div>
        {PROP_QUESTIONS.map((q,i)=>{
          ...
          return<div key={q.id} style={{background:filled?"#f0fdf4":"#f8faff",border:"1px solid "+(filled?"#bbf7d0":"#e2e8f0"),borderRadius:10,padding:"12px 14px",marginBottom:10,transition:"all .2s"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
              <p style={{fontSize:12,fontWeight:700,color:"#1D428A",margin:0}}>Q{i+1} ÃÂ· +{PTS.prop}pts</p>
              {filled&&<span style={{fontSize:10,color:"#15803d",fontWeight:700}}>Ã¢ÂÂ Answered</span>}
            ...
            {q.type==="player"&&<div className="dd-wrap"><select className="sel" value={val} onChange={e=>setObProps(p=>({...p,[q.id]:e.target.value}))} style={{borderColor:filled?"#bbf7d0":"#e2e8f0"}}><option value="">Select playerÃ¢ÂÂ¦</option>{ALL_PLAYERS.map(({p,t})=><option key={p+t} value={p}>{p} ({t})</option>)}</select></div>}
            {q.type==="team"&&<select className="sel" value={val} onChange={e=>setObProps(p=>({...p,[q.id]:e.target.value}))} style={{borderColor:filled?"#bbf7d0":"#e2e8f0"}}><option value="">Select teamÃ¢ÂÂ¦</option>{TEAMS.map(t=><option key={t} value={t}>{TF[t]}</option>)}</select>}
            {q.type==="yesno"&&<div style={{display:"flex",gap:8}}><button className={"bq-btn yes"+(val==="true"?" on":"")} onClick={()=>setObProps(p=>({...p,[q.id]:"true"}))}>Ã¢ÂÂ Yes</button><button className={"bq-btn no"+(val==="false"?" on":"")} onClick={()=>setObProps(p=>({...p,[q.id]:"false"}))}>Ã¢ÂÂ No</button></div>}
          </div>;
        })}
        <button style={{width:"100%",padding:"12px",borderRadius:10,background:"#f1f5f9",color:"#475569",border:"1px solid #e2e8f0",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:15,textTransform:"uppercase",marginBottom:10}} onClick={()=>updateObStep(1,obSp,obT4)}>Ã¢ÂÂ Back</button>
        <button className="lbtn"
          ...
          Lock All Picks Ã¢ÂÂ Let's Play! Ã°ÂÂÂ
        </button>
      </>}
    </div>
    {toast&&<Tst t={toast}/>}
  ...
  if(sc==="propbets"){
    ...
    async function submitStandalonePropBets(){
      ...
      setSc("home");toast2("Prop bets locked! Good luck Ã°ÂÂÂ","ok");
    }
    return<div className="app" style={{minHeight:"100vh",paddingBottom:68}}><style>{CSS}</style>
      <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",padding:"24px 20px 20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <span style={{color:"rgba(255,255,255,.5)",fontSize:12}}>({PROP_QUESTIONS.filter((q,i)=>obProps[`q${i}`]&&obProps[`q${i}`]!=="").length}/{PROP_QUESTIONS.length} answered)</span>
        </div>
        <p style={{color:"#bfdbfe",fontSize:12,margin:0}}>Hey {user?.name} Ã¢ÂÂ one more thing!</p>
      ...
      <div style={{padding:"20px 16px"}}>
        ...
        {PROP_QUESTIONS.map((q,i)=>{
          ...
          return<div key={q.id} style={{background:filled?"#f0fdf4":"#f8faff",border:"1px solid "+(filled?"#bbf7d0":"#e2e8f0"),borderRadius:10,padding:"12px 14px",marginBottom:10,transition:"all .2s"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
              <p style={{fontSize:12,fontWeight:700,color:"#1D428A",margin:0}}>Q{i+1} ÃÂ· +{PTS.prop}pts</p>
              {filled?<span style={{fontSize:10,color:"#15803d",fontWeight:700}}>Ã¢ÂÂ Answered</span>:<span style={{fontSize:10,color:"#ef4444",fontWeight:600}}>Required</span>}
            </div>
            <p style={{fontSize:13,color:"#1a2540",fontWeight:600,margin:"0 0 10px",lineHeight:1.4}}>{q.label}</p>
            {q.type==="player"&&<select className="sel" value={val} onChange={e=>setObProps(p=>({...p,[q.id]:e.target.value}))} style={{borderColor:filled?"#bbf7d0":"#e2e8f0"}}><option value="">Select playerÃ¢ÂÂ¦</option>{ALL_PLAYERS.map(({p,t})=><option key={p+t} value={p}>{p} ({t})</option>)}</select>}
            {q.type==="team"&&<select className="sel" value={val} onChange={e=>setObProps(p=>({...p,[q.id]:e.target.value}))} style={{borderColor:filled?"#bbf7d0":"#e2e8f0"}}><option value="">Select teamÃ¢ÂÂ¦</option>{TEAMS.map(t=><option key={t} value={t}>{TF[t]}</option>)}</select>}
            {q.type==="yesno"&&<div style={{display:"flex",gap:8}}><button className={"bq-btn yes"+(val==="true"?" on":"")} onClick={()=>setObProps(p=>({...p,[q.id]:"true"}))}>Ã¢ÂÂ Yes</button><button className={"bq-btn no"+(val==="false"?" on":"")} onClick={()=>setObProps(p=>({...p,[q.id]:"false"}))}>Ã¢ÂÂ No</button></div>}
          </div>;
        })}
        <button className="lbtn" disabled={!allFilled} onClick={submitStandalonePropBets} style={{opacity:allFilled?1:.4,marginTop:8}}>
          Lock Prop Bets Ã°ÂÂÂ
        </button>
  ...
  if(sc==="picks"&&am){
    ...
    return<div className="app" style={{paddingBottom:32}}><style>{CSS}</style>
      <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",padding:"16px",display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{flex:1}}><p className="C" style={{color:"#fff",fontSize:16,fontWeight:800,margin:0}}>{am.home} vs {am.away}</p><p style={{color:"#bfdbfe",fontSize:11,margin:"2px 0 0"}}>{am.date} ÃÂ· {am.time} IST</p></div>
        <TLogo t={am.away} sz={28}/>
      </div>
      <div style={{background:"#FFF9E6",padding:"8px 16px",borderBottom:"1px solid #FDE68A"}}><span style={{color:"#92400E",fontSize:12}}>Ã¢ÂÂ Ã¯Â¸Â Once submitted, predictions are final. No edits allowed.</span></div>
      <div style={{padding:"16px",display:"flex",flexDirection:"column",gap:18}}>
        ...
        <div>
          ...
          {!draft.sb&&<p style={{fontSize:10,color:"#ef4444",margin:"6px 0 0",textAlign:"center",fontWeight:600}}>Ã¢ÂÂ Ã¯Â¸Â Required</p>}
        </div>
        ...
        {hasBQ&&<div>
          ...
          {draft.bqAns===null&&<p style={{fontSize:10,color:"#ef4444",margin:"4px 0 0",textAlign:"center",fontWeight:600}}>Ã¢ÂÂ Ã¯Â¸Â Required</p>}
        ...
        <button className="lbtn" disabled={!allReady} onClick={submitPick} style={{opacity:allReady?1:.4}}>Lock Prediction Ã°ÂÂÂ</button>
      </div>
      {toast&&<Tst t={toast}/>}
    </div>;
  }
  ...
  if(maintenance&&!isAdmin)return<div className="app"><style>{CSS}</style><div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}><span style={{fontSize:48,marginBottom:16}}>Ã°ÂÂÂ</span><p className="C" style={{color:"#1D428A",fontSize:26,fontWeight:800,letterSpacing:2}}>MAINTENANCE MODE</p><p style={{color:"#64748b",fontSize:14,marginTop:8}}>The app is temporarily offline.</p><button onClick={logout} style={{marginTop:24,padding:"10px 24px",borderRadius:10,background:"#f1f5f9",color:"#64748b",border:"1px solid #e2e8f0",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600,fontSize:13}}>Ã°ÂÂÂª Sign Out</button></div></div>;

  /* -------- MAIN SHELL -------- */
  return<div className="app" style={{paddingBottom:68}}><style>{CSS}</style>
    {hdr}
        {pinnedBc&&<div style={{background:"#1D428A",padding:"8px 16px",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:14}}>Ã°ÂÂÂ¢</span><p style={{color:"#fff",fontSize:12,fontWeight:600,margin:0,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pinnedBc}</p></div>}
    {bc.length>0&&sc==="home"&&!pinnedBc&&(()=>{const lt=bc[bc.length-1];return<div style={{background:"#FFF9E6",borderBottom:"1px solid #FDE68A",padding:"8px 16px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setBcSeenTs(Date.now())}><span style={{color:"#B8860B",fontSize:14}}>Ã°ÂÂÂ</span><p style={{color:"#92400E",fontSize:12,fontWeight:600,margin:0,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{lt.msg}</p>{unbc>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:12}}>{unbc} new</span>}</div>;})()}
    <div style={{background:"#fff",padding:"8px 16px",display:"flex",borderBottom:"1px solid #e2e8f0"}}>
      {[["Ã°ÂÂªÂ","Toss",PTS.toss],["Ã°ÂÂÂ","Win",PTS.win],["Ã¢Â­Â","POTM",PTS.motm],["Ã°ÂÂÂ¥","Streak",PTS.streak],["Ã°ÂÂÂ","Band",PTS.scoreBand],["Ã°ÂÂÂ","Bonus",PTS.bonus]].map(([ic,l,p],i)=><div key={l} style={{flex:1,textAlign:"center",borderRight:i<5?"1px solid #e2e8f0":"none"}}><p style={{color:"#1D428A",fontWeight:700,fontSize:11,margin:0}}>{p}<span style={{fontSize:8,color:"#94a3b8",fontWeight:400}}> pts</span></p><p style={{color:"#64748b",fontSize:8,margin:"1px 0 0"}}>{ic} {l}</p></div>)}
    </div>

    {sc==="home"&&<>
      {/* Prop bets reminder banner Ã¢ÂÂ shows if user skipped the prop bets prompt */}
      {email&&email!==SUPER_ADMIN&&(()=>{
        ...
        return<div style={{background:"#FEF3C7",borderBottom:"1px solid #FDE68A",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
          <div>
            <p style={{color:"#92400E",fontSize:12,fontWeight:700,margin:0}}>Ã¢ÂÂ Ã¯Â¸Â Season Prop Bets not answered ({answeredCount}/5)</p>
            <p style={{color:"#B8860B",fontSize:11,margin:"2px 0 0"}}>Miss out on up to +{PTS.prop*5}pts Ã¢ÂÂ answer now!</p>
          </div>
          <button onClick={()=>{propBetsSkipped.current=false;setSc("propbets");}} style={{background:"#1D428A",color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0,fontFamily:"'Barlow Condensed',sans-serif"}}>Answer Ã¢ÂÂ</button>
        </div>;
      })()}
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {htab==="done"&&(done.length===0?<div style={{textAlign:"center",padding:"48px 16px"}}><p style={{fontSize:40,marginBottom:12}}>Ã°ÂÂÂ­</p><p className="C" style={{color:"#94a3b8",fontSize:18,fontWeight:700,letterSpacing:1}}>NO RESULTS YET</p></div>:[...done].reverse().map(m=><MCard key={m.id} m={m} pred={false} {...cardProps}/>))}
        {htab==="up"&&(upMs.length===0?<div style={{textAlign:"center",padding:"48px 16px"}}><p className="C" style={{color:"#94a3b8",fontSize:16,fontWeight:700}}>ALL MATCHES DONE</p></div>:upMs.map(m=>{
          ...
          return<div key={m.id} style={{background:"#fff",border:"1px solid "+(hasPick?"#bbf7d0":"#e2e8f0"),borderRadius:14,padding:"14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{color:"#94a3b8",fontSize:11,fontWeight:600}}>{m.mn} ÃÂ· {m.date} ÃÂ· {m.time}</span>{hasPick?<span style={{background:"#f0fdf4",color:"#15803d",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Ã¢ÂÂ Predicted</span>:<span style={{background:"#f1f5f9",color:"#64748b",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Upcoming</span>}</div>
            ...
            <p style={{color:"#cbd5e1",fontSize:11,marginTop:10,borderTop:"1px solid #f1f5f9",paddingTop:8}}>Ã°ÂÂÂ {m.venue}</p>
          </div>;
        }))}
        {htab==="season"&&<div>
          ...
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}><p className="st">{(selectedTournament?.name||"IPL 2026").toUpperCase()} CHAMPION</p><div style={{display:"flex",alignItems:"center",gap:14}}>{mySp?<TLogo t={mySp} sz={50}/>:<div style={{width:50,height:50,borderRadius:10,background:"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>Ã°ÂÂÂ</div>}<div><p className="C" style={{color:"#1a2540",fontSize:18,fontWeight:800,margin:0}}>{mySp||"Not set"}</p>{sw&&mySp&&<p style={{color:mySp===sw?"#15803d":"#dc2626",fontSize:13,fontWeight:700,marginTop:6}}>{mySp===sw?"Ã¢ÂÂ Correct! +200pts":"Ã°ÂÂÂ Better luck next time"}</p>}{!sw&&mySp&&<p style={{color:"#94a3b8",fontSize:11,marginTop:4}}>Worth +{PTS.season}pts at season end</p>}</div></div></div>
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}><p className="st">MY TOP 4 PICKS</p>{myT4&&myT4.length>0?<div style={{display:"flex",gap:10,flexWrap:"wrap"}}>{myT4.map((t,i)=><div key={t} style={{display:"flex",alignItems:"center",gap:8,background:"#f8faff",borderRadius:10,padding:"8px 12px",border:"1px solid #e2e8f0"}}><span className="C" style={{color:"#94a3b8",fontSize:13,fontWeight:700}}>#{i+1}</span><TLogo t={t} sz={28}/><span className="C" style={{color:"#1D428A",fontSize:14,fontWeight:700}}>{t}</span>{sw&&<span style={{fontSize:13}}>{t===sw?"Ã¢ÂÂ":"Ã¢ÂÂ"}</span>}</div>)}</div>:<p style={{color:"#94a3b8",fontSize:12}}>Not set</p>}</div>
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px"}}>
            <p className="st">MY SEASON PROP BETS ÃÂ· +{PTS.prop}pts each ÃÂ· {PTS.prop*5}pts total</p>
            {PROP_QUESTIONS.map((q,i)=>{
              ...
              return<div key={q.id} style={{paddingBottom:10,marginBottom:10,borderBottom:"1px solid #f1f5f9"}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  ...
                  {isCorrect&&<span style={{color:"#15803d",fontSize:12,fontWeight:700}}>Ã¢ÂÂ Correct! +{PTS.prop}pts</span>}
                  {isWrong&&<span style={{color:"#dc2626",fontSize:12,fontWeight:700}}>Ã¢ÂÂ Wrong (was: {correctAns})</span>}
      ...
      {getLb().map((u,i)=>(
        <div key={u.email} style={{background:u.email===email?"#EBF0FA":"#fff",border:"1px solid "+(u.email===email?"#1D428A60":"#e2e8f0"),borderRadius:12,padding:"12px 14px",marginBottom:10,boxShadow:"0 1px 4px rgba(29,66,138,.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:32,flexShrink:0}}><span style={{fontSize:i<3?18:13}}>{i===0?"Ã°ÂÂ¥Â":i===1?"Ã°ÂÂ¥Â":i===2?"Ã°ÂÂ¥Â":"#"+(i+1)}</span>{i<3&&<span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:10,color:i===0?"#D4AF37":i===1?"#94a3b8":"#b45309"}}>#{i+1}</span>}</div>
            <Av name={u.name} sz={30}/>
            <div style={{flex:1,minWidth:0}}><div style={{display:"flex",alignItems:"center",gap:5}}><p style={{color:"#1a2540",fontWeight:600,fontSize:13,margin:0}}>{u.name}{u.email===email?" (You)":""}</p>{u.hot&&<span style={{fontSize:13}}>Ã°ÂÂÂ´</span>}</div><div style={{display:"flex",alignItems:"center",gap:6,marginTop:2,flexWrap:"wrap"}}><span style={{fontSize:10,color:"#64748b"}}>{u.acc}% accurate</span>{(u.bgs||[]).slice(0,2).map(b=><span key={b.id} className="bp">{b.ic} {b.lb}</span>)}</div></div>
          ...
          <div style={{display:"flex",gap:8,borderTop:"1px solid #f1f5f9",paddingTop:8,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:5,background:"#f8faff",borderRadius:8,padding:"4px 8px",border:"1px solid #e2e8f0"}}><span style={{fontSize:9,color:"#94a3b8",fontWeight:600,textTransform:"uppercase"}}>Ã°ÂÂÂ</span>{u.userSp?<><TLogo t={u.userSp} sz={16}/><span className="C" style={{fontSize:12,fontWeight:700,color:sw&&u.userSp===sw?"#15803d":"#1D428A"}}>{u.userSp}{sw&&u.userSp===sw?" Ã¢ÂÂ":""}</span></>:<span style={{fontSize:11,color:"#94a3b8"}}>Ã¢ÂÂ</span>}</div>
            <div style={{display:"flex",alignItems:"center",gap:4,background:"#f8faff",borderRadius:8,padding:"4px 8px",border:"1px solid #e2e8f0",flex:1,flexWrap:"wrap"}}><span style={{fontSize:9,color:"#94a3b8",fontWeight:600,textTransform:"uppercase"}}>Top4:</span>{(u.userT4||[]).length>0?(u.userT4||[]).map(t=>{const correct=actualTop4.length>0&&actualTop4.includes(t);const wrong=actualTop4.length>0&&!actualTop4.includes(t);return<div key={t} style={{display:"flex",alignItems:"center",gap:2,background:correct?"#f0fdf4":wrong?"#fef2f2":"#f1f5f9",borderRadius:6,padding:"1px 4px",border:"1px solid "+(correct?"#bbf7d0":wrong?"#fecaca":"#e2e8f0")}}><TLogo t={t} sz={14}/><span style={{fontSize:9,fontWeight:700,color:correct?"#15803d":wrong?"#dc2626":"#475569"}}>{t}</span><span style={{fontSize:9}}>{correct?"Ã¢ÂÂ":wrong?"Ã¢ÂÂ":""}</span></div>;}):<span style={{fontSize:11,color:"#94a3b8"}}>Ã¢ÂÂ</span>}{actualTop4.length>0&&(u.userT4||[]).length>0&&<span style={{fontSize:9,fontWeight:700,color:"#1D428A",marginLeft:4}}>{(u.userT4||[]).filter(t=>actualTop4.includes(t)).length*PTS.top4}pts</span>}</div>
          </div>
          {/* Prop bets row */}
...
{(()=>{
            ...
            if(!hasSomeProps)return<div style={{borderTop:"1px solid #f1f5f9",paddingTop:6,marginTop:4}}><span style={{fontSize:10,color:"#94a3b8",fontStyle:"italic"}}>Ã¢ÂÂ³ Prop bets not yet answered</span></div>;
            const propLabels=["Ã°ÂÂÂ  Orange Cap","Ã°ÂÂÂ£ Purple Cap","Ã°ÂÂÂ Hi Total","Ã°ÂÂÂ¯ Super Over","Ã°ÂÂ¥Â Last Place"];
            ...
            return<div style={{borderTop:"1px solid #f1f5f9",paddingTop:6,marginTop:4}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}>
                <p style={{fontSize:9,color:"#94a3b8",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:0}}>Ã°ÂÂÂ² Season Prop Bets ÃÂ· {propAnsweredCount}/{PROP_QUESTIONS.length} answered</p>
                {propAnsweredCount>0&&<span style={{fontSize:10,fontWeight:800,color:propPtsEarned>0?"#15803d":"#94a3b8",fontFamily:"'Barlow Condensed',sans-serif"}}>{propPtsEarned>0?"+"+propPtsEarned+"pts":"0pts"}{Object.values(propAnswers||{}).some(v=>v)?"":" ÃÂ· TBD"}</span>}
              </div>
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                {PROP_QUESTIONS.map((q,i)=>{
                  ...
                  return<div key={q.id} style={{display:"flex",alignItems:"center",gap:3,background:isCorrect?"#f0fdf4":isWrong?"#fef2f2":isPending?"#FFFBEB":"#f8faff",border:"1px solid "+(isCorrect?"#bbf7d0":isWrong?"#fecaca":isPending?"#FDE68A":"#e2e8f0"),borderRadius:6,padding:"2px 6px"}}>
                    <span style={{fontSize:9,color:"#94a3b8",fontWeight:600}}>{q.short||propLabels[i]}:</span>
                    <span style={{fontSize:10,fontWeight:700,color:isCorrect?"#15803d":isWrong?"#dc2626":isPending?"#92400E":"#94a3b8"}}>{shortVal||"Ã¢ÂÂ"}</span>
                    {isCorrect&&<span style={{fontSize:9,color:"#15803d",fontWeight:700}}> +{PTS.prop}pts</span>}
                    {isWrong&&<span style={{fontSize:9,color:"#dc2626"}}>Ã¢ÂÂ</span>}
                    {isPending&&<span style={{fontSize:9,color:"#B45309"}}>Ã¢ÂÂ³</span>}
                  </div>;
                })}
              </div>
            </div>;
      ...
      {getLb().length===0&&<div style={{textAlign:"center",padding:"48px 16px"}}><p style={{fontSize:36}}>Ã°ÂÂÂ</p><p style={{color:"#94a3b8",marginTop:12}}>No players yet.</p></div>}
    </div>}
    ...
    {sc==="picks"&&!am&&(()=>{
      ...
      return<div style={{padding:"16px"}}>
        <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",borderRadius:14,padding:"16px",marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:6}}>{[["Ã°ÂÂÂ",rows.length,"Played"],["Ã°ÂÂÂ¯",totalPts,"Pts"],["Ã°ÂÂÂ",perfect,"Perfect"],["Ã°ÂÂÂ",acc+"%","Acc"]].map(([ic,val,lbl])=><div key={lbl} className="stat-mini"><p style={{fontSize:14,margin:0}}>{ic}</p><p className="C" style={{color:"#FFE57F",fontSize:15,fontWeight:800,margin:"2px 0 0"}}>{val}</p><p style={{color:"rgba(255,255,255,.6)",fontSize:9,margin:0,textTransform:"uppercase",letterSpacing:.3}}>{lbl}</p></div>)}</div>
          {rows.length>0&&<>
            <div style={{borderTop:"1px solid rgba(255,255,255,.15)",marginTop:10,paddingTop:10}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {[["Ã°ÂÂªÂ",tossAcc+"%","Toss"],["Ã°ÂÂÂ",winAcc+"%","Winner"],["Ã¢Â­Â",motmAcc+"%","POTM"]].map(([ic,val,lbl])=><div key={lbl} className="stat-mini"><p style={{fontSize:12,margin:0}}>{ic}</p><p className="C" style={{color:"#FFE57F",fontSize:14,fontWeight:800,margin:"2px 0 0"}}>{val}</p><p style={{color:"rgba(255,255,255,.5)",fontSize:9,margin:0,textTransform:"uppercase",letterSpacing:.3}}>{lbl}</p></div>)}
                {streakBest>0&&<div className="stat-mini"><p style={{fontSize:12,margin:0}}>Ã°ÂÂÂ¥</p><p className="C" style={{color:"#FFE57F",fontSize:14,fontWeight:800,margin:"2px 0 0"}}>{streakBest}</p><p style={{color:"rgba(255,255,255,.5)",fontSize:9,margin:0,textTransform:"uppercase",letterSpacing:.3}}>Best Streak</p></div>}
              </div>
            </div>
          </>}
        </div>
        ...
        {ptab==="pending"&&(pending.length===0?<div style={{textAlign:"center",padding:"32px 16px"}}><p style={{fontSize:36}}>Ã°ÂÂÂ­</p><p style={{color:"#94a3b8",marginTop:8,fontSize:13}}>No pending predictions.</p></div>:pending.map(m=>{const p=getP(myPicks,m.id);return<div key={m.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{color:"#94a3b8",fontSize:11,fontWeight:600}}>{m.mn} ÃÂ· {m.date} ÃÂ· {m.time}</span><span style={{background:"#f0fdf4",color:"#15803d",fontSize:10,padding:"3px 9px",borderRadius:20,fontWeight:600}}>Ã°ÂÂÂ Locked</span></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><TLogo t={m.home} sz={32}/><span className="C" style={{color:"#94a3b8",fontSize:14,fontWeight:700}}>VS</span><TLogo t={m.away} sz={32}/></div><div style={{background:"#f0fdf4",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#15803d"}}>
                  <p style={{fontSize:10,fontWeight:700,color:"#15803d",textTransform:"uppercase",letterSpacing:.5,margin:"0 0 8px"}}>Your Predictions</p>           <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>             {[["Ã°ÂÂªÂ Toss",p?.toss],["Ã°ÂÂÂ Winner",p?.win],["Ã¢Â­Â POTM",p?.motm?.split(" ").slice(-1)[0]||"Ã¢ÂÂ"],["Ã°ÂÂÂ 1st Innings",p?.sb?SCORE_BANDS.find(b=>b.id===p.sb)?.short||p.sb:"Ã¢ÂÂ"]].map(([l,v])=>(               <div key={l} style={{background:"rgba(255,255,255,.7)",borderRadius:8,padding:"6px 8px"}}>                 <p style={{fontSize:9,color:"#64748b",fontWeight:600,margin:0}}>{l}</p>                 <p style={{fontSize:12,fontWeight:700,color:"#1a2540",margin:"2px 0 0"}}>{v||"Ã¢ÂÂ"}</p>               </div>             ))}           </div>           {(()=>{const myBQ=myBonusPicks[String(m.id)];return myBQ!=null             ?<div style={{background:"rgba(255,255,255,.7)",borderRadius:8,padding:"6px 8px",marginTop:6}}>                 <p style={{fontSize:9,color:"#64748b",fontWeight:600,margin:0}}>Ã°ÂÂÂ Bonus Q</p>                 <p style={{fontSize:12,fontWeight:700,color:"#1a2540",margin:"2px 0 0"}}>{myBQ?"Yes":"No"}</p>               </div>             :null;})()}
                  {p?.sb&&<span> ÃÂ· Ã°ÂÂÂ {SCORE_BANDS.find(b=>b.id===p.sb)?.short||p.sb}</span>}
                  {(()=>{const myBQ=myBonusPicks[String(m.id)];return myBQ!=null?<span> ÃÂ· ? {myBQ?"Yes":"No"}</span>:null;})()}
                </div></div>;}))}{ptab==="played"&&(played.length===0?<div style={{textAlign:"center",padding:"32px 16px"}}><p style={{fontSize:36}}>Ã°ÂÂÂ­</p><p style={{color:"#94a3b8",marginTop:8,fontSize:13}}>No results yet.</p></div>:[...rows].reverse().map(({m,p,tossOk,winOk,motmOk,isPerfect,pts,mult,tA,wA,mA,sbOk,sbAns,bqOk,bqAns,myBQ})=><div key={m.id} style={{background:"#fff",border:"1px solid "+(isPerfect?"#bbf7d0":"#e2e8f0"),borderRadius:12,padding:"14px",marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{color:"#94a3b8",fontSize:11,fontWeight:600}}>{m.mn} ÃÂ· {m.date}</span><div style={{display:"flex",gap:6,alignItems:"center"}}>{isPerfect&&<span style={{fontSize:11}}>Ã°ÂÂÂ Perfect</span>}{mult>1&&<span style={{background:"#FF822A",color:"#fff",fontSize:9,padding:"2px 6px",borderRadius:10,fontWeight:700}}>2ÃÂ</span>}<span className="C" style={{color:pts>0?"#15803d":"#94a3b8",fontSize:14,fontWeight:700}}>+{pts}pts</span></div></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[["Toss",p?.toss,m.result?.toss,tossOk,tA],["Win",p?.win,m.result?.win,winOk,wA],["POTM",p?.motm?.split(" ").slice(-1)[0],m.result?.motm?.split(" ").slice(-1)[0],motmOk,mA]].map(([l,pv,rv,ok,avail])=><div key={l} style={{flex:1,minWidth:60,background:!avail?"#f1f5f9":ok?"#f0fdf4":"#fef2f2",borderRadius:8,padding:"6px 8px",textAlign:"center"}}><p style={{fontSize:9,color:"#94a3b8",margin:0,textTransform:"uppercase"}}>{l}</p><p style={{fontSize:11,fontWeight:700,color:!avail?"#94a3b8":ok?"#15803d":"#dc2626",margin:"2px 0 0"}}>{pv||"Ã¢ÂÂ"}</p>{!avail?<p style={{fontSize:9,color:"#94a3b8",margin:"1px 0 0"}}>N/A</p>:<p style={{fontSize:9,color:"#94a3b8",margin:"1px 0 0"}}>{ok?"Ã¢ÂÂ":"Ã¢ÂÂ"} {rv||"NR"}</p>}</div>)}{p?.sb&&<div style={{flex:1,minWidth:60,background:sbAns?(sbOk?"#f0fdf4":"#fef2f2"):"#f1f5f9",borderRadius:8,padding:"6px 8px",textAlign:"center"}}><p style={{fontSize:9,color:"#94a3b8",margin:0,textTransform:"uppercase"}}>Ã°ÂÂÂ Band</p><p style={{fontSize:11,fontWeight:700,color:sbAns?(sbOk?"#15803d":"#dc2626"):"#1a2540",margin:"2px 0 0"}}>{SCORE_BANDS.find(b=>b.id===p.sb)?.short||p.sb}</p><p style={{fontSize:9,color:"#94a3b8",margin:"1px 0 0"}}>{sbAns?(sbOk?"Ã¢ÂÂ +"+PTS.scoreBand:"Ã¢ÂÂ"):"TBD"}</p></div>}{myBQ!=null&&<div style={{flex:1,minWidth:60,background:bqAns!=null?(bqOk?"#f0fdf4":"#fef2f2"):"#f1f5f9",borderRadius:8,padding:"6px 8px",textAlign:"center"}}><p style={{fontSize:9,color:"#94a3b8",margin:0,textTransform:"uppercase"}}>Ã°ÂÂÂ Bonus</p><p style={{fontSize:11,fontWeight:700,color:bqAns!=null?(bqOk?"#15803d":"#dc2626"):"#1a2540",margin:"2px 0 0"}}>{myBQ?"Yes":"No"}</p><p style={{fontSize:9,color:"#94a3b8",margin:"1px 0 0"}}>{bqAns!=null?(bqOk?"Ã¢ÂÂ +"+PTS.bonus:"Ã¢ÂÂ"):"TBD"}</p></div>}</div></div>))}
        {ptab==="upcoming"&&(schedule.length===0?<div style={{textAlign:"center",padding:"32px 16px"}}><p style={{color:"#94a3b8",fontSize:13}}>No upcoming matches.</p></div>:schedule.map(m=>{
          ...
          return<div key={m.id} style={{background:"#fff",border:"1px solid "+(hasPick?"#bbf7d0":"#e2e8f0"),borderRadius:12,padding:"12px 14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <span style={{color:"#94a3b8",fontSize:11,fontWeight:600}}>{m.mn} ÃÂ· {m.date} ÃÂ· {m.time}</span>
              <div style={{display:"flex",gap:5,alignItems:"center"}}>
                {!lk&&<button onClick={()=>toggleReminder(m.id)} title={hasRem?"Cancel reminder":"Set 30-min reminder"} style={{background:hasRem?"#EBF0FA":"#f8faff",border:"1px solid "+(hasRem?"#1D428A":"#e2e8f0"),borderRadius:8,padding:"3px 8px",cursor:"pointer",fontSize:12,color:hasRem?"#1D428A":"#94a3b8"}}>{hasRem?"Ã°ÂÂÂ":"Ã°ÂÂÂ"}</button>}
                {hasPick?<span style={{background:"#f0fdf4",color:"#15803d",fontSize:10,padding:"3px 8px",borderRadius:12,fontWeight:600}}>Ã¢ÂÂ</span>:lk?<span style={{background:"#fee2e2",color:"#991b1b",fontSize:10,padding:"3px 8px",borderRadius:12,fontWeight:600}}>Ã°ÂÂÂ</span>:<span style={{background:"#f1f5f9",color:"#64748b",fontSize:10,padding:"3px 8px",borderRadius:12,fontWeight:600}}>Pending</span>}
              </div>
            </div>
            ...
            {!hasPick&&!lk&&<button className="pbtn" style={{marginTop:10,fontSize:12,padding:"8px"}} onClick={()=>{setAm(m);setDraft({});setSc("picks");}}>Predict Ã¢ÂÂ</button>}
          </div>;
        }))}</div>;
    })()}
    ...
    {sc==="chat"&&<div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 130px)"}}>
      <div style={{background:"#fff",padding:"10px 16px",borderBottom:"1px solid #e2e8f0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div><p style={{fontWeight:700,fontSize:14,color:"#1a2540",margin:0}}>Group Chat</p><p style={{color:"#94a3b8",fontSize:11,margin:0}}>{Object.keys(onlineUsers).length} online ÃÂ· {chat.length}/{CHAT_CAP} messages</p></div>
        {chatMuted&&<span style={{background:"#fef2f2",color:"#dc2626",fontSize:11,padding:"3px 8px",borderRadius:8,fontWeight:600}}>Ã°ÂÂÂ Muted</span>}
      </div>
        ...
        {chat.map(msg=>{
          ...
          return<div key={msg.id} className={"chat-row"+(isSys?" sys":isMe?" me":" them")}>
            ...
            <div style={{display:"flex",alignItems:"flex-end",gap:6,flexDirection:isMe?"row-reverse":"row"}}>
              ...
              {isAdmin&&!isSys&&<button onClick={()=>delMsg(msg.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#fca5a5",fontSize:12,padding:"0 4px",opacity:.6}}>Ã°ÂÂÂÃ¯Â¸Â</button>}
            </div>
      ...
      <div style={{padding:"12px 14px",background:"#fff",borderTop:"1px solid #e2e8f0",display:"flex",gap:10,alignItems:"flex-end"}}>
        <textarea className="inp" value={chatIn} onChange={e=>setChatIn(e.target.value.slice(0,CHAT_MAX))} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();}}} placeholder={chatMuted||(mutedUsers||{})[myEk]?"Chat is disabledÃ¢ÂÂ¦":"Type a messageÃ¢ÂÂ¦"} disabled={chatMuted||(mutedUsers||{})[myEk]} style={{flex:1,resize:"none",minHeight:42,maxHeight:80}}/>
        <button onClick={sendChat} disabled={!chatIn.trim()||chatMuted||(mutedUsers||{})[myEk]} style={{padding:"10px 16px",borderRadius:10,background:"#1D428A",color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontSize:14,flexShrink:0}}>Ã°ÂÂÂ¤</button>
      </div>
    </div>}

    {sc==="wof"&&<div style={{padding:"16px"}}>
      <div style={{background:"linear-gradient(135deg,#D4AF37,#F0C060)",borderRadius:14,padding:"16px",marginBottom:16,textAlign:"center"}}><p className="C" style={{color:"#1a2540",fontSize:24,fontWeight:800,letterSpacing:2,margin:0}}>Ã°ÂÂÂ WALL OF FAME</p><p style={{color:"#5a4000",fontSize:12,marginTop:4}}>Perfect prediction history</p></div>
      {(()=>{
        const perfs=[];
        done.forEach(m=>{
          const tA=!isNR(m.result.toss),wA=!isNR(m.result.win),mA=!isNR(m.result.motm);
        ...
        if(perfs.length===0)return<div style={{textAlign:"center",padding:"48px 16px"}}><p style={{fontSize:36}}>Ã°ÂÂÂ</p><p style={{color:"#94a3b8",marginTop:12}}>No perfect predictions yet.</p></div>;
        return perfs.reverse().map(({user:u,match:m},i)=>(
          <div key={i} style={{background:"#fff",border:"1px solid #FDE68A",borderRadius:12,padding:"14px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <p style={{color:"#64748b",fontSize:12,margin:"2px 0 0"}}>{m.mn}: {m.home} vs {m.away} ÃÂ· {m.date}</p>
    ...
    {sc==="rules"&&<div style={{padding:"16px"}}>
      ...
      {[
        ["Ã°ÂÂÂ Points System",`Toss Winner: +${PTS.toss}pts | Match Winner: +${PTS.win}pts | Player of the Match: +${PTS.motm}pts | All 3 Correct (Streak Bonus): +${PTS.streak}pts extra`],
        ["Ã¢ÂÂ¡ Double Header",`One match per doubleheader day earns 2ÃÂ all points including the Bonus Question and Score Band. Watch for the Ã¢ÂÂ¡ badge on match cards.`],
        ["Ã°ÂÂÂ Season Picks",`${selectedTournament?.name||"IPL 2026"} Champion: +${PTS.season}pts if correct | Top 4 Picks: +${PTS.top4}pts each team that qualifies. Set once during onboarding Ã¢ÂÂ cannot be changed.`],
        ["Ã°ÂÂÂ First Innings Score",`Predict which run band the first batting team will score in. This is compulsory Ã¢ÂÂ you cannot lock a prediction without answering it.\n${SCORE_BANDS.map(b=>"Ã¢ÂÂ¢ Ã°ÂÂÂ "+b.label).join("\n")}\nCorrect band = +${PTS.scoreBand}pts. Admin enters the correct band in the Results tab after the match.`],
        ["Ã°ÂÂÂ Bonus Question",`Every match has one Yes/No bonus question worth +${PTS.bonus}pts. Answer before the match locks Ã¢ÂÂ it appears on each match card. Admin enters the correct answer after the match.`],
        ["Ã°ÂÂÂ­ Pick Reveal Theatre",`After a match locks, tap the Ã°ÂÂÂ­ REVEAL PICKS button on any match card to see a cinematic reveal of everyone's picks Ã¢ÂÂ with highlights for perfects, 0-scorers, and lone wolf winners.`],
        ["Ã°ÂÂÂ² Season Prop Bets",`${PROP_QUESTIONS.length} compulsory season-long questions answered once during onboarding Ã¢ÂÂ all must be answered before you can start playing:\n${PROP_QUESTIONS.map(q=>"Ã¢ÂÂ¢ "+q.label+": +"+PTS.prop+"pts").join("\n")}\nAdmin enters correct answers at season end. Total potential: +${PTS.prop*PROP_QUESTIONS.length}pts.`],
        ["Ã¢ÂÂ° Lock Times",`Predictions lock 35 minutes before match start. The Score Band and Bonus Question also lock at the same time. No changes allowed after lock.`],
        ["Ã°ÂÂÂ Group Leans",`After lock (before result): see how the group voted on Toss and Winner. After result: see full pick splits with actual counts.`],
        ["Ã°ÂÂÂ Match Reminders",`Tap Ã°ÂÂÂ on any upcoming match in My Game Ã¢ÂÂ Schedule to set a 30-minute reminder before lock.`],
      ].map(([t,d])=>(
      ...
      <div style={{background:"#FFF9E6",border:"1px solid #FDE68A",borderRadius:12,padding:"14px",marginBottom:10}}>
        <p style={{fontWeight:700,fontSize:13,color:"#92400E",margin:"0 0 8px"}}>Ã°ÂÂÂ Full Points Summary</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          {[["Ã°ÂÂªÂ Toss correct",`+${PTS.toss}pts`],["Ã°ÂÂÂ Winner correct",`+${PTS.win}pts`],["Ã¢Â­Â POTM correct",`+${PTS.motm}pts`],["Ã°ÂÂÂ¥ All 3 correct bonus",`+${PTS.streak}pts`],["Ã°ÂÂÂ Score band correct",`+${PTS.scoreBand}pts`],["Ã°ÂÂÂ Bonus Q correct",`+${PTS.bonus}pts`],["Ã¢ÂÂ¡ Double header match","ÃÂ2 all above"],["Ã°ÂÂÂ Champion correct",`+${PTS.season}pts`],["Ã°ÂÂÂ Top 4 team correct",`+${PTS.top4}pts each`],["Ã°ÂÂÂ² Prop bet correct",`+${PTS.prop}pts each`]].map(([l,v])=>(
	...
	{sc==="adm"&&isAdmin&&<div style={{padding:"16px"}}>
      <div style={{background:"linear-gradient(135deg,#1a2540,#1D428A)",borderRadius:14,padding:"14px",marginBottom:14,textAlign:"center"}}><p className="C" style={{color:"#FFE57F",fontSize:22,fontWeight:800,letterSpacing:2,margin:0}}>Ã°ÂÂÂ CRICKET ADMIN</p></div>

      {/* Admin tab bar */}
      <div style={{display:"flex",gap:0,background:"#fff",borderRadius:10,border:"1px solid #e2e8f0",marginBottom:14,overflow:"auto"}}>
        {[["approvals","Ã¢ÂÂ Approve"],["manpick","Ã¢ÂÂÃ¯Â¸Â Pick Entry"],["results","Ã°ÂÂÂ Results"],["pickstatus","Ã°ÂÂÂ Pick Status"],["users","Ã°ÂÂÂ¥ Users"],["analytics","Ã°ÂÂÂ Analytics"],["controls","Ã¢ÂÂÃ¯Â¸Â Controls"],["broadcast","Ã°ÂÂÂ¢ Broadcast"]].map(([t,l])=><button key={t} className={"at"+(admTab===t?" on":"")} onClick={()=>setAdmTab(t)}>{l}{t==="approvals"&&pendingCount>0?` (${pendingCount})`:""}</button>)}
      </div>
      ...
      {/* -- RESULTS TAB -- */}
      {admTab==="results"&&<div>
        {/* Repair DB button Ã¢ÂÂ always visible at top of results */}
        <div className="ac" style={{background:"#EBF0FA",border:"2px solid #1D428A"}}>
          <p className="st">Ã°ÂÂÂ§ DB REPAIR TOOL</p>
          <p style={{fontSize:12,color:"#64748b",marginBottom:10}}>Safely re-normalises all pick keys. The fixed code now <b>recovers</b> Firebase-coerced arrays rather than wiping them. Run only when needed Ã¢ÂÂ not automatic anymore.</p>
          <button className="pbtn" disabled={repairLoading} onClick={adminRepairDB}>
            {repairLoading?"RepairingÃ¢ÂÂ¦":"Ã°ÂÂÂ§ Repair & Reload All Picks"}
          </button>
        </div>
        {ms.filter(m=>!isTBD(m)).sort((a,b)=>Number(a.id)-Number(b.id)).map(m=>(
          <div key={m.id} className="ac">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div><p style={{fontWeight:700,fontSize:13,color:"#1a2540",margin:0}}>{m.mn}: {m.home} vs {m.away}</p><p style={{color:"#94a3b8",fontSize:11,margin:0}}>{m.date} ÃÂ· {m.time}</p></div>
              <div style={{display:"flex",gap:6}}>
              </div>
            </div>
            {/* Match result: entry form OR done display */}
            {m.result
              ?<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#15803d",marginBottom:8}}>Ã¢ÂÂ Result: {showVal(m.result.toss)} ÃÂ· {showVal(m.result.win)} ÃÂ· {showVal(m.result.motm)}</div>
              :<div style={{marginBottom:8}}>
                ...
                }}>Ã¢ÂÂ Finalise Result</button>
              </div>
            }

            {/* Q4: Score Band Ã¢ÂÂ always visible so admin can enter before or after result */}
            {(()=>{
              ...
              return<div style={{background:"#F4F6FB",border:"1px solid "+(cur?"#1D428A30":"#e2e8f0"),borderRadius:8,padding:"8px 10px",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                  <p style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:0}}>Ã°ÂÂÂ Q4 ÃÂ· First Innings Score Band</p>
                  {!cur&&<span style={{fontSize:9,color:"#ef4444",fontWeight:700,marginLeft:"auto"}}>Not set</span>}
                  {cur&&<span style={{fontSize:9,color:"#15803d",fontWeight:700,marginLeft:"auto"}}>Ã¢ÂÂ Set: {SCORE_BANDS.find(b=>b.id===cur)?.label}</span>}
                </div>
                <div style={{display:"flex",gap:6}}>
                  ...
                  {cur&&<button onClick={clearSb} style={{padding:"6px 10px",borderRadius:8,background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",cursor:"pointer",fontSize:11,fontWeight:700}}>Ã°ÂÂÂÃ¯Â¸Â</button>}
                </div>
              </div>;
            })()}

            {/* Q5: Bonus Question Ã¢ÂÂ always visible */}
            {BONUS_QUESTIONS[m.id]&&(()=>{
              ...
              return<div style={{background:"#F4F6FB",border:"1px solid "+(bAns!=null?"#1D428A30":"#e2e8f0"),borderRadius:8,padding:"8px 10px",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                  <p style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:0}}>Ã°ÂÂÂ Q5 ÃÂ· Bonus Question</p>
                  {bAns==null&&<span style={{fontSize:9,color:"#ef4444",fontWeight:700,marginLeft:"auto"}}>Not set</span>}
                  {bAns!=null&&<span style={{fontSize:9,color:"#15803d",fontWeight:700,marginLeft:"auto"}}>Ã¢ÂÂ Set: {bAns?"YES":"NO"}</span>}
                </div>
                <p style={{fontSize:11,color:"#1a2540",margin:"0 0 6px",lineHeight:1.4,fontStyle:"italic"}}>{BONUS_QUESTIONS[m.id]}</p>
                <div style={{display:"flex",gap:6}}>
                  {[true,false].map(v=><button key={String(v)} onClick={()=>saveBonusAns(v)} style={{flex:1,padding:"6px",borderRadius:8,background:bAns===v?(v?"#f0fdf4":"#fef2f2"):"#f1f5f9",color:bAns===v?(v?"#15803d":"#dc2626"):"#475569",border:"1px solid "+(bAns===v?(v?"#bbf7d0":"#fecaca"):"#e2e8f0"),cursor:"pointer",fontSize:12,fontWeight:700}}>{v?"Ã¢ÂÂ YES":"Ã¢ÂÂ NO"}</button>)}
                  {bAns!=null&&<button onClick={clearBonusAns} style={{padding:"6px 10px",borderRadius:8,background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",cursor:"pointer",fontSize:11,fontWeight:700}}>Ã°ÂÂÂÃ¯Â¸Â</button>}
                </div>
              </div>;
            })()}

            {m.result&&<button onClick={()=>undoResult(m.id)} style={{marginTop:4,width:"100%",padding:"7px",borderRadius:8,background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",cursor:"pointer",fontSize:11,fontWeight:700}}>Ã¢ÂÂÃ¯Â¸Â Edit / Undo Result</button>}
          </div>
        ))}
      </div>}
      ...
      {/* -- USERS TAB -- */}
      {admTab==="users"&&<div>
        <input className="inp" placeholder="Search usersÃ¢ÂÂ¦" value={userSearch} onChange={e=>setUserSearch(e.target.value)} style={{marginBottom:12}}/>
        {Object.values(users).filter(u=>u?.email&&u.approved!==false&&(u.name.toLowerCase().includes(userSearch.toLowerCase())||u.email.toLowerCase().includes(userSearch.toLowerCase()))).map(u=>{
          ...
          return<div key={u.email} className="ac" style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setExU(open?null:u.email)}>
              ...
              <div style={{flex:1}}><p style={{fontWeight:700,fontSize:13,color:"#1a2540",margin:0}}>{u.name}{u.email===email?" (You)":""}</p><p style={{color:"#94a3b8",fontSize:11,margin:0}}>{u.email} ÃÂ· {pickCount} picks</p></div>
              <span style={{color:"#94a3b8",fontSize:12}}>{open?"Ã¢ÂÂ²":"Ã¢ÂÂ¼"}</span>
            </div>
            {open&&<div style={{marginTop:12,borderTop:"1px solid #f1f5f9",paddingTop:12}}>
              ...
              {u.email!==SUPER_ADMIN&&<button className="dbtn" onClick={()=>deleteUser(u.email)}>Ã°ÂÂÂÃ¯Â¸Â Delete User</button>}
            </div>}
          </div>;
        })}
      </div>}
      ...
      {admTab==="analytics"&&(()=>{
        ...
        return<div>
          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            {[["Ã°ÂÂÂ",doneMs.length,"Matches Done"],["Ã°ÂÂÂ",totalPerfects2,"Group Perfects"],["Ã°ÂÂªÂ",avgTossAcc+"%","Avg Toss Acc"],["Ã°ÂÂÂ",avgWinAcc+"%","Avg Win Acc"],["Ã¢Â­Â",avgMotmAcc+"%","Avg POTM Acc"]].map(([ic,val,lbl])=>(
          ...
          {upsets.length>0&&<div className="ac" style={{marginBottom:12}}>
            <p className="st" style={{marginBottom:8}}>Ã°ÂÂÂ± BIGGEST UPSETS (Group mostly wrong)</p>
            {upsets.map(s=><div key={s.m.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #f1f5f9"}}>
              <div><p style={{fontSize:12,fontWeight:600,color:"#1a2540",margin:0}}>{s.m.mn}: {s.m.home} vs {s.m.away}</p><p style={{fontSize:10,color:"#94a3b8",margin:0}}>Winner: <b style={{color:"#1a2540"}}>{s.m.result.win}</b> ÃÂ· {s.tot} picks</p></div>
          ...
          {easiest.length>0&&<div className="ac" style={{marginBottom:12}}>
            <p className="st" style={{marginBottom:8}}>Ã°ÂÂÂ¯ MOST PREDICTED CORRECTLY</p>
          ...
          <div className="ac">
            <p className="st" style={{marginBottom:8}}>Ã°ÂÂÂ¯ ALL MATCH ACCURACY (WINNER %)</p>
      ...
      {admTab==="controls"&&<div>
        ...
        <div className="ac">
          ...
          <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>Ã°ÂÂÂ Match Lock Controls</p>
            ...
            {ms.filter(m=>!isTBD(m)&&!m.result).map(m=>{
              ...
              return<div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderBottom:"1px solid #f1f5f9"}}>
                ...
                <button onClick={()=>toggleMatchLock(m.id)} style={{padding:"4px 10px",borderRadius:8,background:st==="locked"?"#fee2e2":st==="unlocked"?"#f0fdf4":"#f1f5f9",color:st==="locked"?"#991b1b":st==="unlocked"?"#15803d":"#475569",border:"1px solid "+(st==="locked"?"#fecaca":st==="unlocked"?"#bbf7d0":"#e2e8f0"),cursor:"pointer",fontSize:11,fontWeight:600}}>
                  {st==="locked"?"Ã°ÂÂÂ Locked":st==="unlocked"?"Ã°ÂÂÂ Unlocked":"Ã¢ÂÂ±Ã¯Â¸Â Auto"}
                </button>
              </div>;
            })}
          </div>
          <p style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>Ã¢ÂÂ¡ DOUBLE HEADER MATCH</p>
          <p style={{fontSize:12,color:"#64748b",marginBottom:10}}>Select which match gets 2ÃÂ points multiplier.</p>
        ...
        <div className="ac">
          <p className="st">Ã°ÂÂÂ ACTUAL TOP 4 TEAMS</p>
          ...
          {actualTop4.length===4&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#15803d",marginBottom:8}}>Ã¢ÂÂ Top 4 set: {actualTop4.join(", ")}</div>}
          {actualTop4.length>0&&actualTop4.length<4&&<div style={{background:"#FEF3C7",border:"1px solid #FDE68A",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#92400E",marginBottom:8}}>Ã¢ÂÂ¹Ã¯Â¸Â Select {4-actualTop4.length} more team{4-actualTop4.length!==1?"s":""}</div>}
          {actualTop4.length>0&&<button className="dbtn" style={{marginBottom:14}} onClick={async()=>{setActualTop4([]);await DB.set("actualtop4",[]);toast2("Top 4 cleared");}}>Ã°ÂÂÂÃ¯Â¸Â Clear Top 4</button>}
        </div>
        <div className="ac">
        ...
        <div className="ac">
          ...
          <p style={{fontSize:12,color:"#64748b",marginBottom:10}}>Enter correct answers at season end Ã¢ÂÂ each correct user pick = +{PTS.prop}pts.</p>
          {PROP_QUESTIONS.map((q,i)=>{
            ...
            return<div key={q.id} style={{marginBottom:12}}>
              <p style={{fontSize:11,fontWeight:700,color:"#1D428A",margin:"0 0 3px"}}>Q{i+1} ÃÂ· {q.label}</p>
              {q.type==="player"&&<select className="sel" value={cur} onChange={e=>savePropAns(e.target.value)}><option value="">Select correct playerÃ¢ÂÂ¦</option>{ALL_PLAYERS.map(({p,t})=><option key={p+t} value={p}>{p} ({t})</option>)}</select>}
              {q.type==="team"&&<select className="sel" value={cur} onChange={e=>savePropAns(e.target.value)}><option value="">Select correct teamÃ¢ÂÂ¦</option>{TEAMS.map(t=><option key={t} value={t}>{TF[t]}</option>)}</select>}
              {q.type==="yesno"&&<div style={{display:"flex",gap:8}}>{["true","false"].map(v=><button key={v} onClick={()=>savePropAns(v)} style={{flex:1,padding:"7px",borderRadius:8,background:cur===v?(v==="true"?"#f0fdf4":"#fef2f2"):"#f1f5f9",color:cur===v?(v==="true"?"#15803d":"#dc2626"):"#475569",border:"1px solid "+(cur===v?(v==="true"?"#bbf7d0":"#fecaca"):"#e2e8f0"),cursor:"pointer",fontSize:12,fontWeight:700}}>{v==="true"?"Ã¢ÂÂ Yes":"Ã¢ÂÂ No"}</button>)}</div>}
            </div>;
          })}
        </div>
        <div className="ac">
          ...
          <div style={{display:"flex",gap:8,flexDirection:"column"}}>
            <button className="pbtn" onClick={exportCSV}>Ã°ÂÂÂ¥ Export Leaderboard CSV</button>
            <button className="pbtn" style={{background:"linear-gradient(135deg,#0f6e56,#1D9E75)"}} onClick={exportPicksCSV}>Ã°ÂÂÂ¥ Export All Picks CSV</button>
          </div>
        </div>
        <div className="ac" style={{background:"#EBF0FA",border:"2px solid #1D428A"}}>
          <p className="st">Ã°ÂÂÂ§ DB REPAIR</p>
      ...
      {admTab==="broadcast"&&<div>
        <div className="ac">
          ...
          <textarea className="inp" value={bcMsg} onChange={e=>setBcMsg(e.target.value.slice(0,300))} placeholder="Type announcementÃ¢ÂÂ¦" style={{minHeight:80,resize:"none",marginBottom:4}}/>
          <div className="charcnt">{bcMsg.length}/300</div>
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button className="pbtn" style={{flex:1}} onClick={()=>sendBc(false)}>Ã°ÂÂÂ¤ Send</button>
            <button className="pbtn" style={{flex:1,background:"linear-gradient(135deg,#D4AF37,#F0C060)",color:"#1a2540"}} onClick={()=>sendBc(true)}>Ã°ÂÂÂ Pin</button>
          </div>
          {pinnedBc&&<button className="dbtn" style={{marginTop:8}} onClick={clearPin}>Ã°ÂÂÂÃ¯Â¸Â Clear Pinned Message</button>}
        </div>
        <div className="ac">
