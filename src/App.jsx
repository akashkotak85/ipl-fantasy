...
try{localStorage.removeItem("ipl26_session");}catch(e){}if(!cancelled)setSc("login");return;}
  ...
  */
  ...
  if(sc==="splash")return<div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f2456,#1D428A,#2a5bbf)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 20px"}}><style>{CSS}</style><div style={{width:80,height:80,borderRadius:20,background:"linear-gradient(135deg,#FFE57F,#F0C040)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20,boxShadow:"0 8px 32px rgba(255,215,0,.35)"}}><p style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:34,color:"#1D428A",margin:0,letterSpacing:1}}>FSP</p></div><p className="C" style={{fontSize:32,fontWeight:800,color:"#fff",letterSpacing:3,margin:0}}>FANTASY SPORTS</p><p className="C" style={{fontSize:20,fontWeight:700,color:"#fff",letterSpacing:3,margin:"2px 0 0"}}>PREDICTOR</p><p style={{color:"#FFE57F",fontSize:11,letterSpacing:4,marginTop:8,marginBottom:40,textTransform:"uppercase"}}>CRICKET · FOOTBALL · MORE</p><div style={{display:"flex",alignItems:"center",gap:8,color:"rgba(255,255,255,.5)",fontSize:12}}><div className="spin" style={{width:14,height:14,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%"}}/> Loading…</div></div>;
