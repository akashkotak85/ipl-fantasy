    ...
    {sc==="lb"&&<div style={{padding:"16px"}}>
      ...
      {getLb().map((u,i)=>(
        <div key={u.email} style={{background:u.email===email?"#EBF0FA":"#fff",border:"1px solid "+(u.email===email?"#1D428A60":"#e2e8f0"),borderRadius:12,padding:"12px 14px",marginBottom:10,boxShadow:"0 1px 4px rgba(29,66,138,.06)"}}>
          ...
          {/* Prop bets row */}
          {(()=>{const mAdj=getManualAdj(u.email)+getMatchOverride(u.email);return<div style={{borderTop:"1px solid #f1f5f9",paddingTop:6,marginTop:4,display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:9,color:"#94a3b8",fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>📈 Stock Market</span>{mAdj>0?<span style={{fontSize:12,fontWeight:800,fontFamily:"'Barlow Condensed',sans-serif",color:"#15803d"}}>+{mAdj}pts</span>:<span style={{fontSize:12,fontWeight:500,color:"#94a3b8"}}>0pts</span>}</div>;})()}
{(()=>{
