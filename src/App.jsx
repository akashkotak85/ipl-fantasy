...
/* ─── MATCH RESULTS (confirmed completed matches only — M1–M7) ─── */
const KNOWN_RESULTS = {
  ...
  // M8 (DC vs MI 15:30) and M9 (GT vs RR 19:30) are TODAY — enter via Admin → Results
};
...
/* ─── UTILS ─── */
// FIX: single canonical email encoder used everywhere
...
function deepEncodeKeys(v){
  ...
  Object.keys(v).forEach(k=>{
    // FIX: only encode if looks like an email (contains @ or already encoded _at_)
    const shouldEncode = k.includes("@") || k.includes(".");
    r[shouldEncode ? encodeEmail(k) : k] = deepEncodeKeys(v[k]);
  });
  return r;
}
...
const DB={
  ...
  set:async(k,v)=>{
    try{
      const{db,dbMod}=await firebaseReady;
      // FIX: only deep-encode top-level objects that are keyed by email
      const emailKeyedPaths=["u","ap","sp","t4","ptsadj","matchptsoverride","mutedusers","online"];
      const shouldEncode=emailKeyedPaths.some(p=>k===p||k.startsWith(p+"/"));
      const sv=(shouldEncode&&typeof v==="object"&&v!==null&&!Array.isArray(v))?deepEncodeKeys(v):v;
      if(sv===null||sv===undefined){
        await dbMod.remove(dbMod.ref(db,PFX+k));
      }else{
        await dbMod.set(dbMod.ref(db,PFX+k),sv);
      }
...
const cutoff=m=>{
  ...
  return d?new Date(d-45*60*1000):new Date(0);
};
...
// FIX: robust POTM matching — trim, lowercase, handle partial name
const motmMatch=(a,b)=>{
  if(!a||!b)return false;
...
/* ─── SCORING
   FIX: allPicks is stored with encoded email keys. calcScore takes uPicks directly (already decoded for this user).
   lbScores must look up by encoded key consistently.
─── */
function calcScore(uPicks,ms,dbl=null){
  let pts=0,ok=0,tot=0,ms2={};
  ms.forEach(m=>{
    if(!m.result)return;
    // FIX: always check both numeric id and string id
...
function calcBadges(uPicks,ms,allP){
  ...
  // underdog: picked a winner that <50% of group picked
  let ud=0;
  done.forEach(m=>{
    const p=uPicks[m.id]??uPicks[String(m.id)];
    if(!p||p.win!==m.result.win)return;
    const allArr=Object.values(allP);
    const t2=allArr.filter(u=>u[m.id]||u[String(m.id)]).length||1;
    const agreers=allArr.filter(u=>(u[m.id]??u[String(m.id)])?.win===m.result.win).length;
    if(agreers/t2<0.5)ud++;
  });
  ...
  return <div style={{marginBottom:10}}>
    ...
    <div style={{display:"flex",gap:4,alignItems:"center"}}>
      ...
      <div style={{flex:1,height:7,borderRadius:4,overflow:"hidden",display:"flex"}}>
        <div style={{width:pA+"%",background:clA,transition:"width .6s"}}/>
        <div style={{flex:1,background:clB}}/>
      </div>
      <span style={{fontSize:11,fontWeight:700,color:"#1a2540",minWidth:28}}>{100-pA}%</span>
    </div>
    <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
      <span style={{fontSize:10,color:"#94a3b8"}}>{tA}</span>
      <span style={{fontSize:10,color:"#94a3b8"}}>{tB}</span>
    </div>
  </div>;
}
  ...
  return <div className="dd-wrap" ref={ref}>
    ...
    {open&&<div className="dd-list">
      {players.map(({p,t})=>{
        ...
        return <div key={p} className={"dd-item"+(value===p?" sel":"")} onMouseDown={e=>{e.preventDefault();onChange(p);setOpen(false);}}>
...
function MCard({m,pred,myPicks,allPicks,rxns,reminders,doubleMatch,lockedMatches,matchPtsOverride,email,onPredict,onReminder,onReact}){
  ...
  // FIX: always check both numeric and string id
  ...
  // FIX: strict number comparison for doubleMatch
  ...
  if(m.result&&mp){
    let base=0;
    if(mp.toss===m.result.toss)base+=PTS.toss;
    if(mp.win===m.result.win)base+=PTS.win;
    if(motmMatch(mp.motm,m.result.motm))base+=PTS.motm;
    if(mp.toss===m.result.toss&&mp.win===m.result.win&&motmMatch(mp.motm,m.result.motm))base+=PTS.streak;
    earned=base*mult;
  }
  ...
  const sp=lk&&tot>0?{
    tot,
    tA:allArr.filter(u=>(u[m.id]??u[String(m.id)])?.toss===m.home).length,
    tB:tot-allArr.filter(u=>(u[m.id]??u[String(m.id)])?.toss===m.home).length,
    wA:allArr.filter(u=>(u[m.id]??u[String(m.id)])?.win===m.home).length,
    wB:tot-allArr.filter(u=>(u[m.id]??u[String(m.id)])?.win===m.home).length
  }:null;
  ...
  return <div className="mcard fade-in">
    ...
    {m.result&&<div style={{background:"#F4F6FB",borderRadius:8,padding:"8px 12px",fontSize:12,marginBottom:8}}>
      <span style={{color:"#64748b"}}>Toss: </span><b>{m.result.toss}</b>
    ...
    {mp&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"7px 12px",fontSize:12,color:"#15803d",marginBottom:8}}>
      Locked: {mp.toss} toss · {mp.win} win · POTM: {mp.motm?.split(" ").slice(-1)[0]}
    </div>}
    ...
    {pred&&lk&&!mp&&!m.result&&<div style={{textAlign:"center",padding:"8px",fontSize:12,color:"#991b1b",marginTop:4}}>🔒 Locked — prediction window closed</div>}
  </div>;
}
...
export default function App(){
  ...
  // FIX: seed BASE_MATCHES with KNOWN_RESULTS so matches show as done immediately
  const buildBaseMatches=useCallback(()=>{
    return BASE_MATCHES.map(m=>{
      const r=KNOWN_RESULTS[m.id];
      return r?{...m,result:r,status:"completed"}:{...m,result:null};
    });
  },[]);
  ...
  const toast2=useCallback((msg,type="info")=>{
    setToast({msg,type});
  ...
  const reloadShared=useCallback(async(em)=>{
    ...
    // FIX: normalize allPicks — ensure we always use encoded keys
    const rawAP=ap||{};
    const freshAP={};
    Object.keys(rawAP).forEach(k=>{
      const normalizedKey=k.includes("@")?ek(k):k; // encode if raw email slipped through
      freshAP[normalizedKey]=rawAP[k];
    });
    setAllPicks(freshAP);
    if(em)setMyPicks(freshAP[emk]||{});

    // Build match list: base (with known results) + firebase overrides + extra manual matches
    const base=buildBaseMatches();
    let allMs=[...base];

    // FIX: apply firebase rm overrides on top (admin-entered results take priority over KNOWN_RESULTS)
    if(rm){
      allMs=allMs.map(m=>{
  ...
  useEffect(()=>{
    if(["home","picks","lb","wof","adm","hist"].includes(sc)&&email)reloadShared(email);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[sc]);
  ...
  useEffect(()=>{
    ...
    Object.keys(reminders).forEach(mid=>{
      ...
      const m=ms.find(x=>x.id===parseInt(mid)||x.id===mid);
      if(!m)return;
      const diff=cutoff(m).getTime()-30*60*1000-Date.now();
      if(diff>0&&diff<24*60*60*1000){
        remTimers.current[mid]=setTimeout(()=>toast2("⏰ "+m.home+" vs "+m.away+" locks in 30 mins!"),diff);
      }
    });
  ...
  const getMatchOverride=useCallback(em=>{
    const emk=ek(em);
    const byEncoded=matchPtsOverride[emk]||{};
    const byRaw=matchPtsOverride[em]||{};
    const merged={...byRaw,...byEncoded};
    return Object.values(merged).reduce((a,b)=>a+b,0);
  },[matchPtsOverride]);

  const myS=useMemo(()=>calcScore(myPicks,ms,doubleMatch),[myPicks,ms,doubleMatch]);
  const myPts=useMemo(()=>{
    const spPts=(spk[myEk]&&sw&&spk[myEk]===sw)?PTS.season:0;
    const t4Pts=(sw&&myT4&&myT4.includes(sw))?PTS.top4:0;
    return myS.pts+spPts+t4Pts+getManualAdj(email)+getMatchOverride(email);
  },[myS,spk,myEk,sw,myT4,getManualAdj,getMatchOverride,email]);

  /* ─── LEADERBOARD SCORES
     FIX: key lookup uses encoded email consistently
  ─── */
  const lbScores=useMemo(()=>{
    ...
    Object.values(users).forEach(u=>{
      ...
      // FIX: look up picks by encoded key only (all stored as encoded)
      ...
      const adj=getManualAdj(u.email);
      const mOv=getMatchOverride(u.email);
      scores[u.email]={
        pts:st.pts+sp2+t4p+adj+mOv,
        acc:st.acc,
        hot:st.hot,
  ...
  const getLb=useCallback(()=>
    Object.values(users)
      .filter(u=>u?.email)
      .map(u=>({...u,...(lbScores[u.email]||{pts:0,acc:0,hot:false,bgs:[],userSp:"",userT4:[]})}))
      .sort((a,b)=>b.pts-a.pts),
  [users,lbScores]);

  const getWof=useCallback(()=>done.map(m=>{
    const perfs=Object.entries(allPicks).filter(([emk,up])=>{
      const p=up[m.id]??up[String(m.id)];
      return p&&m.result&&p.toss===m.result.toss&&p.win===m.result.win&&motmMatch(p.motm,m.result.motm);
    }).map(([emk])=>{
      // FIX: find user by encoded key
      const u=Object.values(users).find(u=>ek(u.email)===emk);
      return{name:u?.name||emk,email:emk};
    });
    return{...m,perfs};
  }),[done,allPicks,users]);
  ...
  async function doLogin(){
    ...
    try{
      ...
      // FIX: check both raw and encoded key in users object
      const userEntry=u2[em]||u2[ek(em)];
      if(!userEntry||storedHash==null){
        setAuthErrors({email:"No account found. Please create one."});
        setAuthLoading(false);return;
      }
      const inputHash=await sha256(authPw);
      // FIX: support legacy plain-text pw migration
      ...
      // Migrate plain-text pw to hashed
      if(storedHash===authPw)await DB.set("pw_"+ek(em),inputHash);
      setUsers(u2);
      await doSignIn(em,userEntry);
    }catch(e){
  ...
  async function doRegister(){
    ...
    if(regAttempts.current.length>=REG_LIMIT){setAuthErrors({email:"Too many attempts. Try again in 10 minutes."});return;}
    ...
    try{
      ...
      // FIX: verify write succeeded before signing in
  ...
  async function doForgotStep1(){
    ...
    if(eErr){setAuthErrors({email:eErr});setAuthLoading(false);return;}
    const u2=await DB.get("u")||{};
  ...
  async function doForgotStep2(){
    ...
    clearAuthForm();setAuthMode("login");setAuthEmail(savedEmail);
    setAuthLoading(false);
  }
  ...
  async function logout(){
    // FIX: clean up all timers and intervals before clearing state
    Object.keys(remTimers.current).forEach(id=>clearTimeout(remTimers.current[id]));
    remTimers.current={};
    if(pollRef.current){clearInterval(pollRef.current);pollRef.current=null;}
    clearTimeout(tRef.current);
    if(sessionEmail){
      ...
      }catch(e){console.error("logout cleanup",e);}
    }
    setSessionEmail(null);setUser(null);setEmail("");setMyPicks({});setMySp("");setMyT4([]);
    setIsAdmin(false);setAm(null);setAllPicks({});setSpk({});setT4pk({});setOnlineUsers({});
    setUsers({});setBcSeenTs(0);setChatSeenTs(Date.now());setChatU(0);
    setToast(null);
    clearAuthForm();
    setSc("login");
  }
  ...
  async function doneOnboard(){
    ...
    setSc("home");
    toast2("Picks locked! Let the games begin! 🏏","ok");
  }

  /* ─── PICK SUBMISSION ─── */
  async function submitPick(){
    if(!am)return;
    // FIX: re-fetch match state from DB to check if locked
    const freshRm=await DB.get("rm")||{};
    const freshMatch={...am,...(freshRm[am.id]??freshRm[String(am.id)]??{})};
    // Also apply known results
    if(KNOWN_RESULTS[am.id])freshMatch.result=KNOWN_RESULTS[am.id];
    if(isMatchLocked(freshMatch,lockedMatches)){
      toast2("Match is now locked","error");setAm(null);setSc("home");return;
    }
    if(!draft.toss||!draft.win){toast2("Pick toss and winner first","error");return;}
  ...
  async function sendChat(){
    ...
    if(chatMuted){toast2("Chat is muted for everyone","error");return;}
    if((mutedUsers||{})[myEk]){toast2("You have been muted by admin","error");return;}
  ...
  async function delMsg(id){
    const latest=await DB.get("ch")||[];
    const nc=latest.filter(m=>m.id!==id);
  ...
  async function setManualResult(mid){
    ...
    const nm=ms.map(m=>Number(m.id)===Number(mid)?{...m,result,status:"completed"}:m);
    setMs(nm);
    const rm=(await DB.get("rm"))||{};
    rm[numMid]={result,status:"completed"};
    await DB.set("rm",rm);
    // FIX: clear form entry after save
    ...
    const perfs=Object.entries(freshAP).filter(([emk,up])=>{
      const p=up[numMid]??up[String(numMid)];
      return p&&p.toss===result.toss&&p.win===result.win&&motmMatch(p.motm,result.motm);
    }).map(([emk])=>{
      const u=Object.values(cu).find(u=>ek(u.email)===emk);
      return u?.name||emk;
    });
    const matchObj=nm.find(x=>Number(x.id)===Number(mid));
    const latest=await DB.get("ch")||[];
    const newCh=capChat([...latest,{
      id:Date.now(),email:"__sys__",name:"IPL Bot",
      text:"Result: "+matchObj.home+" vs "+matchObj.away+"\nWinner: "+result.win+" · POTM: "+result.motm+(perfs.length?"\n🎯 Perfect: "+perfs.join(", "):"\nNo perfect picks this match"),
  ...
  async function deleteUser(ue){
    ...
    const np={...manualPtsAdj};delete np[ue];delete np[uek];
    const nmpo={...matchPtsOverride};delete nmpo[ue];delete nmpo[uek];
    setUsers(nu);setAllPicks(na);setSpk(ns);setT4pk(nt);setManualPtsAdj(np);setMatchPtsOverride(nmpo);
    await Promise.all([DB.set("u",nu),DB.set("ap",na),DB.set("sp",ns),DB.set("t4",nt),DB.set("ptsadj",np),DB.set("pw_"+uek,null),DB.set("token_"+uek,null),DB.set("session",null),DB.set("matchptsoverride",nmpo)]);
  ...
  async function addManualMatch(){
    ...
    if(home===away){toast2("Home and away teams must differ","error");return;}
    ...
    await DB.set("manmatches",[...existing,nm]);
    setMs(prev=>[...prev,nm]);
    setManMatchForm({mn:"",home:"RCB",away:"MI",date:"",time:"19:30",venue:""});
    toast2("Match added!","ok");
  }
  ...
  async function adjustPts(em,delta){
    const emk=ek(em);
    const cur=manualPtsAdj[emk]||manualPtsAdj[em]||0;
    const upd={...manualPtsAdj,[emk]:cur+delta};
    setManualPtsAdj(upd);await DB.set("ptsadj",upd);
    toast2((delta>0?"+":"")+delta+" pts","ok");
  }

  async function setMatchPts(em,mid,delta){
    const emk=ek(em);
    const cur=((matchPtsOverride[emk]||{})[mid])||((matchPtsOverride[emk]||{})[String(mid)])||0;
    const upd={...matchPtsOverride,[emk]:{...(matchPtsOverride[emk]||{}),[mid]:cur+delta}};
    setMatchPtsOverride(upd);await DB.set("matchptsoverride",upd);
    toast2((delta>0?"+":"")+delta+" pts","ok");
  }
  ...
  async function toggleReminder(mid){
    ...
    toast2(on?"🔔 Reminder set":"🔕 Reminder removed","ok");
  }

  function exportCSV(){
    const lb=getLb();
    const rows=[["Rank","Name","Email","Points","Accuracy","Champion","Top4"].join(","),
      ...lb.map((u,i)=>[i+1,'"'+u.name+'"',u.email,u.pts,u.acc+"%",u.userSp||"",((u.userT4||[]).join("|"))].join(","))
    ];
    const blob=new Blob([rows.join("\n")],{type:"text/csv"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download="ipl26_leaderboard.csv";a.click();
    URL.revokeObjectURL(url);
    toast2("CSV exported!","ok");
  }

  /* ─── SHARED CARD PROPS ─── */
  const cardProps={
    myPicks,allPicks,rxns,reminders,doubleMatch,lockedMatches,matchPtsOverride,email,
    onReminder:toggleReminder,
    onReact:reactFn,
    onPredict:(m)=>{setAm(m);setDraft({});setSc("picks");}
  };
  ...
  const hdr=<div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",padding:"13px 16px 11px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      <div>
        <p className="C" style={{color:"#FFE57F",fontSize:13,fontWeight:700,letterSpacing:1,margin:0,textTransform:"uppercase"}}>Fantasy Predictor{isAdmin?" · Admin":""}</p>
        <p style={{color:"#bfdbfe",fontSize:10,margin:0}}>TATA IPL 2026{maintenance?" · 🔒 Maintenance":""}</p>
      </div>
    </div>
  ...
  if(sc==="splash")return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f2456,#1D428A,#2a5bbf)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
    ...
    <div style={{display:"flex",gap:14,marginBottom:14,justifyContent:"center"}}>
      {["RCB","MI","CSK","KKR","SRH"].map((t,i)=><div key={t} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,animation:`fadeIn .4s ease ${i*.08}s both`}}><div style={{width:52,height:52,borderRadius:12,background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.2)",display:"flex",alignItems:"center",justifyContent:"center"}}><TLogo t={t} sz={38}/></div><span style={{color:"rgba(255,255,255,.7)",fontSize:9,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,letterSpacing:1}}>{t}</span></div>)}
    </div>
    <div style={{display:"flex",gap:14,marginBottom:36,justifyContent:"center"}}>
      {["RR","PBKS","GT","LSG","DC"].map((t,i)=><div key={t} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,animation:`fadeIn .4s ease ${(i+5)*.08}s both`}}><div style={{width:52,height:52,borderRadius:12,background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.2)",display:"flex",alignItems:"center",justifyContent:"center"}}><TLogo t={t} sz={38}/></div><span style={{color:"rgba(255,255,255,.7)",fontSize:9,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,letterSpacing:1}}>{t}</span></div>)}
    </div>
    <div style={{display:"flex",alignItems:"center",gap:8,color:"rgba(255,255,255,.6)",fontSize:12}}>
      <span style={{fontSize:16}} className="spin">⚙</span> Loading…
    </div>
  </div>;
  ...
  if(sc==="login")return <div className="app"><style>{CSS}</style>
    <div style={{background:"linear-gradient(160deg,#0f2456,#1D428A,#2a5bbf)",padding:"32px 24px 28px",textAlign:"center"}}>
      ...
      <div style={{display:"flex",gap:0,marginTop:16,background:"rgba(255,255,255,.1)",borderRadius:12,padding:3}}>
        {[["login","Sign In"],["register","Register"],["forgot","Reset PW"]].map(([m,l])=>(
          <button key={m} onClick={()=>{setAuthMode(m);clearAuthForm();}} style={{flex:1,padding:"8px 4px",borderRadius:9,background:authMode===m?"#fff":"transparent",color:authMode===m?"#1D428A":"rgba(255,255,255,.7)",fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:10,border:"none",cursor:"pointer",textTransform:"uppercase",letterSpacing:.5}}>{l}</button>
        ))}
      </div>
    </div>
    <div style={{padding:"22px 24px",display:"flex",flexDirection:"column",gap:14}}>
      {authMode==="login"&&<>
        <div>
          <input className={"inp"+(authErrors.email?" err":"")} value={authEmail} onChange={e=>{setAuthEmail(e.target.value);setAuthErrors(p=>({...p,email:""}));}} placeholder="Email address" autoCapitalize="off" autoCorrect="off"/>
      ...
      {authMode==="register"&&<>
        ...
        <div>
          ...
          <p style={{color:"#94a3b8",fontSize:10,marginTop:4}}>Min 8 chars · uppercase · number · special char</p>
          {authErrors.pw&&<p className="ferr">{authErrors.pw}</p>}
        </div>
        <div>
          <div style={{position:"relative"}}>
            <input className={"inp"+(authErrors.pw2?" err":"")} type={showPw2?"text":"password"} value={authPw2} onChange={e=>{setAuthPw2(e.target.value);setAuthErrors(p=>({...p,pw2:""}));}} placeholder="Confirm password" style={{paddingRight:48}} onKeyDown={e=>e.key==="Enter"&&doRegister()}/>
      ...
      {authMode==="forgot"&&<>
        {forgotStep===1&&<>
          <div style={{background:"#EBF0FA",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#1e40af"}}>Enter your registered email to reset your password.</div>
          <div>
            <input className={"inp"+(authErrors.email?" err":"")} value={authEmail} onChange={e=>{setAuthEmail(e.target.value);setAuthErrors(p=>({...p,email:""}));}} placeholder="Email address" autoCapitalize="off"/>
            {authErrors.email&&<p className="ferr">{authErrors.email}</p>}
          </div>
          <button className="pbtn" disabled={authLoading} onClick={doForgotStep1}>{authLoading?"Checking…":"Verify Email"}</button>
        </>}
        {forgotStep===2&&<>
          <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#166534"}}>✅ Verified: <b>{authEmail}</b></div>
          <div>
            <div style={{position:"relative"}}>
              <input className={"inp"+(authErrors.pw?" err":"")} type={showForgotPw?"text":"password"} value={forgotNewPw} onChange={e=>{setForgotNewPw(e.target.value);setAuthErrors(p=>({...p,pw:""}));}} placeholder="New password" style={{paddingRight:48}}/>
              <button onClick={()=>setShowForgotPw(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showForgotPw?"🙈":"👁"}</button>
            </div>
            {authErrors.pw&&<p className="ferr">{authErrors.pw}</p>}
          </div>
          <div>
            <div style={{position:"relative"}}>
              <input className={"inp"+(authErrors.pw2?" err":"")} type={showForgotPw2?"text":"password"} value={forgotNewPw2} onChange={e=>{setForgotNewPw2(e.target.value);setAuthErrors(p=>({...p,pw2:""}));}} placeholder="Confirm new password" style={{paddingRight:48}} onKeyDown={e=>e.key==="Enter"&&doForgotStep2()}/>
              <button onClick={()=>setShowForgotPw2(p=>!p)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18}}>{showForgotPw2?"🙈":"👁"}</button>
            </div>
            {authErrors.pw2&&<p className="ferr">{authErrors.pw2}</p>}
          </div>
    ...
    <div style={{padding:"20px 16px"}}>
      {obStep===0&&<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:10,justifyContent:"center",marginBottom:24}}>
          {TEAMS.map(t=><button key={t} className={"ot"+(obSp===t?" on":"")} onClick={()=>setObSp(t)}>
            <TLogo t={t} sz={38}/><span style={{fontSize:11,fontWeight:700,color:obSp===t?"#1D428A":"#475569"}}>{t}</span>
          </button>)}
        </div>
      ...
      {obStep===1&&<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:10,justifyContent:"center",marginBottom:24}}>
          {TEAMS.map(t=>{
            ...
            return <button key={t} className={"ot"+(sel?" on":"")} onClick={()=>{
              if(sel)setObT4(p=>p.filter(x=>x!==t));
              else if(obT4.length<4)setObT4(p=>[...p,t]);
              else toast2("Max 4 teams","error");
            }}>
              <TLogo t={t} sz={38}/>
              <span style={{fontSize:11,fontWeight:700,color:sel?"#1D428A":"#475569"}}>{t}</span>
  ...
  if(sc==="picks"&&am)return <div className="app" style={{paddingBottom:32}}><style>{CSS}</style>
    <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",padding:"16px",display:"flex",alignItems:"center",gap:14}}>
      ...
      <div style={{flex:1}}>
        <p className="C" style={{color:"#fff",fontSize:16,fontWeight:800,margin:0}}>{am.home} vs {am.away}</p>
        <p style={{color:"#bfdbfe",fontSize:11,margin:"2px 0 0"}}>{am.date} · {am.time} IST</p>
      </div>
      <TLogo t={am.away} sz={28}/>
    </div>
    <div style={{background:"#EBF0FA",padding:"8px 16px",borderBottom:"1px solid #dbeafe"}}>
      <span style={{color:"#1D428A",fontSize:12}}>All 3 correct = +{PTS.streak}pts streak bonus · Predictions are final</span>
    </div>
      ...
      {[["TOSS WINNER","toss",PTS.toss],["MATCH WINNER","win",PTS.win]].map(([title,field,pts])=>(
        <div key={field}>
          ...
          <div style={{display:"flex",gap:10}}>
            {[am.home,am.away].map(t=>(
              <button key={t} className={"tmbtn"+(draft[field]===t?" on":"")} onClick={()=>setDraft(d=>({...d,[field]:t}))}>
                <TLogo t={t} sz={50}/>
                <p className="C" style={{color:draft[field]===t?"#1D428A":"#64748b",fontSize:14,fontWeight:700,margin:0}}>{t}</p>
      ...
      {draft.toss&&draft.win&&draft.motm&&(
        <div style={{background:"#EBF0FA",border:"1px solid #dbeafe",borderRadius:12,padding:"14px 16px"}}>
          ...
          <div style={{borderTop:"1px solid #dbeafe",paddingTop:8,marginTop:4,display:"flex",justifyContent:"space-between"}}>
            <span style={{color:"#64748b",fontSize:12}}>Max with streak bonus</span>
            <span className="C" style={{color:"#1D428A",fontSize:18,fontWeight:800}}>+{PTS.toss+PTS.win+PTS.motm+PTS.streak}pts</span>
          </div>
        </div>
      )}
  ...
  return <div className="app" style={{paddingBottom:68}}><style>{CSS}</style>
    ...
    {/* Pinned broadcast */}
    {pinnedBc&&<div style={{background:"#1D428A",padding:"8px 16px",display:"flex",alignItems:"center",gap:10}}>
      <span style={{fontSize:14}}>📌</span>
      <p style={{color:"#fff",fontSize:12,fontWeight:600,margin:0,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pinnedBc}</p>
    </div>}

    {/* Latest broadcast banner (home only) */}
    {bc.length>0&&sc==="home"&&!pinnedBc&&(()=>{
      const lt=bc[bc.length-1];
      return <div style={{background:"#FFF9E6",borderBottom:"1px solid #FDE68A",padding:"8px 16px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setBcSeenTs(Date.now())}>
        <span style={{color:"#B8860B",fontSize:14}}>📢</span>
        <p style={{color:"#92400E",fontSize:12,fontWeight:600,margin:0,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{lt.msg}</p>
        {unbc>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:12}}>{unbc} new</span>}
      </div>;
    })()}

    {/* Points legend */}
    <div style={{background:"#fff",padding:"8px 16px",display:"flex",borderBottom:"1px solid #e2e8f0"}}>
      {[["🎯","Toss",PTS.toss],["🏆","Win",PTS.win],["⭐","POTM",PTS.motm],["🔥","Streak",PTS.streak]].map(([ic,l,p],i)=>(
        <div key={l} style={{flex:1,textAlign:"center",borderRight:i<3?"1px solid #e2e8f0":"none"}}>
          <p style={{color:"#1D428A",fontWeight:700,fontSize:12,margin:0}}>{p}<span style={{fontSize:9,color:"#94a3b8",fontWeight:400}}> pts</span></p>
          <p style={{color:"#64748b",fontSize:9,margin:"1px 0 0"}}>{ic} {l}</p>
        </div>
      ))}
    </div>
    ...
    {sc==="home"&&<>
      <div style={{display:"flex",background:"#fff",borderBottom:"1px solid #e2e8f0"}}>
        {[["today","Today ("+todayMs.length+")"],["done","Results ("+done.length+")"],["up","Schedule ("+upMs.length+")"],["season","Season"]].map(([t,l])=>(
          <button key={t} className={"tbtn"+(htab===t?" on":"")} onClick={()=>setHtab(t)}>{l}</button>
        ))}
      </div>
      <div style={{padding:"14px 14px 0"}}>
        {htab==="today"&&(todayMs.length===0
          ?<div style={{textAlign:"center",padding:"48px 16px"}}><p style={{fontSize:40,marginBottom:12}}>🏏</p><p className="C" style={{color:"#94a3b8",fontSize:18,fontWeight:700,letterSpacing:1}}>NO MATCHES TODAY</p><p style={{color:"#94a3b8",fontSize:12,marginTop:8}}>Check the Schedule tab for upcoming matches.</p></div>
        ...
        {htab==="up"&&(upMs.length===0
          ...
          :upMs.map(m=>(
            <div key={m.id} style={{background:"#fff",border:"1px solid "+(reminders[m.id]?"#FF822A40":"#e2e8f0"),borderRadius:14,padding:"14px",marginBottom:10}}>
              ...
              {/* FIX: Show predict button for upcoming matches too if open */}
        ...
        {htab==="season"&&<div>
          ...
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div>
                <p className="C" style={{color:"#1a2540",fontSize:18,fontWeight:800,margin:0}}>{mySp||"Not set"}</p>
                ...
                {!sw&&mySp&&<p style={{color:"#94a3b8",fontSize:11,marginTop:4}}>Worth +{PTS.season}pts at season end</p>}
              </div>
            </div>
          </div>
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px"}}>
            ...
            {myT4&&myT4.length>0
              ?<div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {myT4.map((t,i)=>(
                  <div key={t} style={{display:"flex",alignItems:"center",gap:8,background:"#f8faff",borderRadius:10,padding:"8px 12px",border:"1px solid #e2e8f0"}}>
                    <span className="C" style={{color:"#94a3b8",fontSize:13,fontWeight:700}}>#{i+1}</span>
                    <TLogo t={t} sz={28}/>
                    <span className="C" style={{color:"#1D428A",fontSize:14,fontWeight:700}}>{t}</span>
                    {sw&&<span style={{fontSize:13}}>{t===sw?"✅":"❌"}</span>}
                  </div>
                ))}
              </div>
      ...
      {getLb().map((u,i)=>(
        <div key={u.email} style={{background:u.email===email?"#EBF0FA":"#fff",border:"1px solid "+(u.email===email?"#1D428A60":"#e2e8f0"),borderRadius:12,padding:"12px 14px",marginBottom:10,boxShadow:"0 1px 4px rgba(29,66,138,.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
            <div style={{width:30,height:30,borderRadius:8,background:i===0?"#D4AF37":i===1?"#94a3b8":i===2?"#b45309":"#EBF0FA",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:12,color:i<3?"#fff":"#475569",flexShrink:0}}>
              {i===0?"🥇":i===1?"🥈":i===2?"🥉":i+1}
            </div>
            <Av name={u.name} sz={30}/>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                <p style={{color:"#1a2540",fontWeight:600,fontSize:13,margin:0}}>{u.name}{u.email===email?" (You)":""}</p>
                {u.hot&&<span style={{fontSize:13}}>🔥</span>}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:6,marginTop:2,flexWrap:"wrap"}}>
                <span style={{fontSize:10,color:"#64748b"}}>{u.acc}% accurate</span>
                {(u.bgs||[]).slice(0,2).map(b=><span key={b.id} className="bp">{b.ic} {b.lb}</span>)}
              </div>
            </div>
    ...
    {sc==="picks"&&!am&&<div style={{padding:"16px"}}>
      <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",borderRadius:14,padding:"14px 16px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <p className="C" style={{color:"#FFE57F",fontSize:20,fontWeight:800,letterSpacing:1,margin:0}}>MY PICKS</p>
          <p style={{color:"#bfdbfe",fontSize:12,marginTop:2}}>{Object.keys(myPicks).length} predictions · {myS.acc}% accurate</p>
        </div>
          ...
          <div style={{flex:1,background:"#f8faff",border:"1px solid #dbeafe",borderRadius:10,padding:"10px 12px"}}>
            ...
            {mySp
              ?<div style={{display:"flex",alignItems:"center",gap:8,marginTop:6}}>
                <TLogo t={mySp} sz={32}/>
                <div>
                  <p className="C" style={{color:"#1D428A",fontSize:16,fontWeight:800,margin:0}}>{mySp}</p>
                  <p style={{color:sw?(mySp===sw?"#15803d":"#dc2626"):"#94a3b8",fontSize:10,fontWeight:600,margin:0}}>{sw?(mySp===sw?"✅ +200pts":"❌ Wrong"):"Season pending"}</p>
                </div>
              </div>
        ...
        <div style={{background:"#f8faff",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 12px"}}>
          ...
          {myT4&&myT4.length>0
            ?<div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>
              {myT4.map((t,i)=>(
                <div key={t} style={{display:"flex",alignItems:"center",gap:6,background:"#fff",borderRadius:10,padding:"6px 10px",border:"1px solid #e2e8f0"}}>
                  <span className="C" style={{color:"#94a3b8",fontSize:11,fontWeight:700}}>#{i+1}</span>
                  <TLogo t={t} sz={22}/>
                  <span className="C" style={{color:"#1D428A",fontSize:12,fontWeight:700}}>{t}</span>
                  {sw&&<span style={{fontSize:11}}>{t===sw?"✅":"❌"}</span>}
                </div>
              ))}
            </div>
            :<p style={{color:"#94a3b8",fontSize:12,margin:"6px 0 0"}}>Not set</p>
      ...
      <div style={{background:"#FFF9E6",border:"1px solid #FDE68A",borderRadius:10,padding:"8px 12px",marginBottom:12,fontSize:12,color:"#92400E",display:"flex",gap:8,alignItems:"center"}}>
        <span>🔒</span><span>Picks lock 45 mins before each match. Predict from Today or Schedule tabs.</span>
      </div>
      {ms.filter(m=>myPicks[m.id]??myPicks[String(m.id)]).length===0
        ?<div style={{textAlign:"center",padding:"40px 16px"}}><p style={{fontSize:36}}>📋</p><p style={{color:"#94a3b8",marginTop:12}}>No predictions yet. Head to the Today tab!</p></div>
        :ms.filter(m=>myPicks[m.id]??myPicks[String(m.id)]).map(m=>{
          ...
          if(m.result){
            let base=0;
            if(p.toss===m.result.toss)base+=PTS.toss;
            if(p.win===m.result.win)base+=PTS.win;
            if(motmMatch(p.motm,m.result.motm))base+=PTS.motm;
            if(p.toss===m.result.toss&&p.win===m.result.win&&motmMatch(p.motm,m.result.motm))base+=PTS.streak;
            e=base*mult;
          }
          const mOv=((matchPtsOverride[email]||{})[m.id])??((matchPtsOverride[email]||{})[String(m.id)])??0;
          return <div key={m.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{color:"#94a3b8",fontSize:11}}>{m.mn} · {m.date}</span>
                {isDouble&&<span style={{fontSize:10,background:"linear-gradient(135deg,#FF822A,#D4AF37)",color:"#fff",borderRadius:10,padding:"1px 6px",fontWeight:700}}>2×</span>}
              </div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {[["Toss",p.toss,m.result&&p.toss===m.result.toss],["Win",p.win,m.result&&p.win===m.result.win],["POTM",p.motm,m.result&&motmMatch(p.motm,m.result.motm)]].map(([l,v,c])=>(
                <span key={l} style={{background:m.result?(c?"#f0fdf4":"#fef2f2"):"#f8faff",border:"1px solid "+(m.result?(c?"#bbf7d0":"#fecaca"):"#e2e8f0"),borderRadius:6,padding:"4px 10px",fontSize:12,color:m.result?(c?"#15803d":"#dc2626"):"#475569"}}>{l}: {v||"—"}</span>
              ))}
            </div>
          </div>;
        })
    ...
    {sc==="hist"&&(()=>{
      ...
      const rows=played.map(m=>{
        ...
        const isDouble=doubleMatch!=null&&Number(doubleMatch)===Number(m.id);
        const mult=isDouble?2:1;
        const tossOk=p.toss===m.result.toss;
        const winOk=p.win===m.result.win;
        const motmOk=motmMatch(p.motm,m.result.motm);
        const isPerfect=tossOk&&winOk&&motmOk;
        let base=0;
        if(tossOk)base+=PTS.toss;if(winOk)base+=PTS.win;if(motmOk)base+=PTS.motm;
        if(isPerfect){base+=PTS.streak;perfect++;streakCur++;streakBest=Math.max(streakBest,streakCur);}else{streakCur=0;}
        const mOv=((matchPtsOverride[email]||{})[m.id])??((matchPtsOverride[email]||{})[String(m.id)])??0;
        const pts=(base*mult)+mOv;
        totalPts+=pts;
      ...
      return <div style={{padding:"16px"}}>
        <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",borderRadius:14,padding:"16px",marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {[["🏏",rows.length,"Played"],["⭐",totalPts,"Pts Earned"],["🎯",perfect,"Perfect"],["📊",acc+"%","Accuracy"]].map(([ic,val,lbl])=>(
              <div key={lbl} style={{flex:1,background:"rgba(255,255,255,.12)",borderRadius:10,padding:"8px 4px",textAlign:"center"}}>
                ...
                <p className="C" style={{color:"#FFE57F",fontSize:16,fontWeight:800,margin:"2px 0 0",letterSpacing:.5}}>{val}</p>
                <p style={{color:"#bfdbfe",fontSize:9,margin:0,textTransform:"uppercase",letterSpacing:.5}}>{lbl}</p>
              </div>
            ))}
          </div>
          {streakBest>=2&&<div style={{marginTop:10,background:"rgba(255,229,127,.15)",borderRadius:8,padding:"6px 12px",display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:14}}>🔥</span><span style={{color:"#FFE57F",fontSize:12,fontWeight:600}}>Best perfect streak: {streakBest} in a row</span>
          </div>}
        </div>
        {pending.length>0&&<div style={{background:"#FFF9E6",border:"1px solid #FDE68A",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
          ...
          {pending.map(m=>{
            const p=myPicks[m.id]??myPicks[String(m.id)];
            const isDouble=doubleMatch!=null&&Number(doubleMatch)===Number(m.id);
            return <div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #FDE68A40"}}>
              <TLogo t={m.home} sz={20}/><span style={{color:"#94a3b8",fontSize:10}}>vs</span><TLogo t={m.away} sz={20}/>
              <div style={{flex:1,minWidth:0}}>
                <p style={{color:"#1a2540",fontSize:12,fontWeight:600,margin:0}}>{m.mn} · {m.home} vs {m.away}{isDouble?" ⚡":""}</p>
                <p style={{color:"#92400E",fontSize:11,margin:"2px 0 0"}}>{p.toss} toss · {p.win} win · {p.motm?.split(" ").slice(-1)[0]}</p>
              </div>
              <span style={{background:"#FDE68A",color:"#92400E",fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:12,flexShrink:0}}>Pending</span>
            </div>;
          })}
        </div>}
        {rows.length===0
          ?<div style={{textAlign:"center",padding:"40px 16px"}}>
            ...
            <p className="C" style={{color:"#94a3b8",fontSize:16,fontWeight:700,letterSpacing:1,marginTop:12}}>NO RESULTS YET</p>
            <p style={{color:"#94a3b8",fontSize:12,marginTop:8}}>Your history appears once matches complete.</p>
          </div>
          :<>
            ...
            {[...rows].reverse().map(({m,p,tossOk,winOk,motmOk,isPerfect,pts,mult,mOv})=>(
              <div key={m.id} style={{background:"#fff",border:"1px solid "+(isPerfect?"#bbf7d0":"#e2e8f0"),borderRadius:12,padding:"12px 14px",marginBottom:10,position:"relative",overflow:"hidden"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{color:"#1a2540",fontSize:12,fontWeight:600,margin:0}}>{m.mn} · {m.date}</p>
                    <p style={{color:"#64748b",fontSize:11,margin:"1px 0 0"}}>Win: <b style={{color:"#15803d"}}>{m.result.win}</b> · POTM: <b style={{color:"#B8860B"}}>{m.result.motm}</b></p>
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <p className="C" style={{color:pts>0?"#15803d":"#94a3b8",fontSize:20,fontWeight:800,margin:0}}>{pts>0?"+":""}{pts}</p>
                    <p style={{color:"#94a3b8",fontSize:9,margin:0,textTransform:"uppercase"}}>pts{mult>1?" (2×)":""}</p>
                  </div>
                </div>
                <div style={{display:"flex",gap:6}}>
                  {[["Toss",p.toss,tossOk,m.result.toss],["Winner",p.win,winOk,m.result.win],["POTM",p.motm?.split(" ").slice(-1)[0],motmOk,m.result.motm?.split(" ").slice(-1)[0]]].map(([lbl,val,ok,actual])=>(
                    <div key={lbl} style={{flex:1,background:ok?"#f0fdf4":"#fef2f2",border:"1px solid "+(ok?"#bbf7d0":"#fecaca"),borderRadius:8,padding:"6px 8px",textAlign:"center"}}>
                      <p style={{color:"#94a3b8",fontSize:9,fontWeight:600,textTransform:"uppercase",margin:"0 0 3px"}}>{lbl}</p>
                      <p style={{color:ok?"#15803d":"#dc2626",fontSize:11,fontWeight:700,margin:0}}>{ok?"✓":"✗"} {val||"—"}</p>
                      {!ok&&<p style={{color:"#94a3b8",fontSize:9,margin:"2px 0 0"}}>was {actual}</p>}
                    </div>
                  ))}
                </div>
                {isPerfect&&<div style={{marginTop:8,background:"#f0fdf4",borderRadius:6,padding:"5px 10px",display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:12}}>🎯</span><span style={{color:"#15803d",fontSize:11,fontWeight:700}}>Perfect! +{PTS.streak}pts streak bonus</span>
                </div>}
    ...
    {sc==="remind"&&<div style={{padding:"16px"}}>
      <div style={{background:"linear-gradient(135deg,#FF822A,#D4AF37)",borderRadius:14,padding:"16px",marginBottom:14,textAlign:"center"}}>
        ...
        <p style={{color:"rgba(255,255,255,.85)",fontSize:12,marginTop:4}}>{remCount} reminder{remCount!==1?"s":""} set</p>
      </div>
      <div style={{display:"flex",background:"#f1f5f9",borderRadius:10,marginBottom:14,overflow:"hidden",border:"1px solid #e2e8f0"}}>
        {[["upcoming","All Upcoming"],["mine","My Reminders ("+remCount+")"]].map(([t,l])=>(
          <button key={t} className={"at"+(remTab===t?" on":"")} onClick={()=>setRemTab(t)}>{l}</button>
        ))}
      </div>
      {(()=>{
        const upcoming=ms.filter(m=>!m.result&&!isTBD(m));
        const list=remTab==="mine"?upcoming.filter(m=>reminders[m.id]):upcoming;
        if(!list.length)return <div style={{textAlign:"center",padding:"40px 16px"}}>
          <span style={{fontSize:36}}>📅</span>
          <p className="C" style={{color:"#94a3b8",fontSize:16,fontWeight:700,letterSpacing:1,marginTop:12}}>{remTab==="mine"?"NO REMINDERS SET":"NO UPCOMING MATCHES"}</p>
        </div>;
        return list.map(m=>{
          const on=!!reminders[m.id];
          const mp=myPicks[m.id]??myPicks[String(m.id)];
          ...
          return <div key={m.id} className="rcard" style={{borderColor:on?"#FF822A40":"#e2e8f0",background:on?"#FFFAF5":"#fff"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{textAlign:"center",flex:2}}>
                <span className="C" style={{color:"#cbd5e1",fontSize:14,fontWeight:800,letterSpacing:2}}>VS</span>
                <div style={{color:"#64748b",fontSize:10,marginTop:2,fontWeight:600}}>{m.mn} · {m.date}</div>
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div style={{flex:1,textAlign:"center"}}>
                {mp
                  ?<span style={{background:"#f0fdf4",color:"#15803d",fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:20,border:"1px solid #bbf7d0"}}>✅ Picked</span>
                  :lk
                    ?<span style={{background:"#f1f5f9",color:"#94a3b8",fontSize:11,fontWeight:600,padding:"4px 10px",borderRadius:20}}>🔒 Locked</span>
              ...
              <button onClick={()=>toggleReminder(m.id)} className={on?"bell-on":""} style={{width:44,height:44,borderRadius:12,background:on?"linear-gradient(135deg,#FF822A,#D4AF37)":"#f8faff",border:"1.5px solid "+(on?"#FF822A":"#e2e8f0"),cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,flexShrink:0}}>
                <span style={{fontSize:18}}>{on?"🔔":"🔕"}</span>
                <span style={{fontSize:8,color:on?"#fff":"#94a3b8",fontWeight:700,fontFamily:"'Barlow',sans-serif",textTransform:"uppercase"}}>{on?"ON":"OFF"}</span>
              </button>
            </div>
          </div>;
        });
    ...
    {sc==="chat"&&<div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 134px)"}}>
      <div style={{padding:"10px 16px 8px",borderBottom:"1px solid #e2e8f0",background:"#fff"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <div>
            ...
            <p style={{color:"#64748b",fontSize:11,margin:"2px 0 0"}}>{Object.keys(users).length} players · <span style={{color:"#22c55e",fontWeight:600}}>{Object.keys(onlineUsers).length} online</span>{chatMuted?" · 🔇 Chat muted":""}</p>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:10,background:"#F4F6FB"}}>
        ...
        {chat.map(msg=>{
          const im=msg.email===email;const is=msg.sys;
          return <div key={msg.id} style={{display:"flex",flexDirection:"column",alignSelf:is?"center":im?"flex-end":"flex-start",maxWidth:"88%",gap:3}}>
            ...
            <div className={"cb "+(is?"cs":im?"cm":"co")} style={{whiteSpace:"pre-line"}}>
              {msg.text}
              {isAdmin&&!is&&<button onClick={()=>delMsg(msg.id)} style={{background:"none",border:"none",color:im?"rgba(255,255,255,.4)":"#94a3b8",fontSize:10,cursor:"pointer",marginLeft:8,padding:0}}>×</button>}
            </div>
      ...
      <div style={{padding:"10px 14px 12px",borderTop:"1px solid #e2e8f0",background:"#fff"}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-end"}}>
          <input
            className="inp"
            value={chatIn}
            onChange={e=>setChatIn(e.target.value.slice(0,CHAT_MAX))}
            placeholder={chatMuted||(mutedUsers||{})[myEk]?"Chat is muted":"Type a message…"}
            disabled={chatMuted||(mutedUsers||{})[myEk]}
    ...
    {sc==="wof"&&<div style={{padding:"16px"}}>
      <div style={{background:"linear-gradient(135deg,#1D428A,#2a5bbf)",borderRadius:14,padding:"16px",marginBottom:16,textAlign:"center"}}>
        ...
        <p style={{color:"#bfdbfe",fontSize:12,marginTop:4}}>Perfect predictions hall of fame</p>
      </div>
      {(()=>{
        const pc={};
        done.forEach(m=>{
          Object.entries(allPicks).forEach(([emk,up])=>{
            const p=up[m.id]??up[String(m.id)];
            if(p&&m.result&&p.toss===m.result.toss&&p.win===m.result.win&&motmMatch(p.motm,m.result.motm)){
              pc[emk]=(pc[emk]||0)+1;
            }
          });
        });
        ...
        return <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:16}}>
          ...
          {s.map(([emk,cnt],i)=>{
            const u=Object.values(users).find(u=>ek(u.email)===emk);
            if(!u)return null;
            return <div key={emk} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:i<s.length-1?"1px solid #f1f5f9":"none"}}>
              ...
              <Av name={u.name} sz={30}/>
              <div style={{flex:1}}><p style={{color:"#1a2540",fontWeight:600,fontSize:13,margin:0}}>{u.name}</p><p style={{color:"#64748b",fontSize:11,margin:0}}>{cnt} perfect match{cnt>1?"es":""}</p></div>
              <span className="C" style={{color:"#1D428A",fontSize:18,fontWeight:800}}>{cnt}×</span>
            </div>;
          })}
        </div>;
      })()}
      {getWof().map(m=>(
        <div key={m.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <TLogo t={m.home} sz={24}/>
            <div style={{flex:1}}>
              <p className="C" style={{color:"#1a2540",fontSize:14,fontWeight:700,margin:0}}>{m.home} vs {m.away}</p>
              <p style={{color:"#64748b",fontSize:11,margin:0}}>{m.mn} · {m.date}</p>
            </div>
            <TLogo t={m.away} sz={24}/>
          </div>
          {m.result&&<div style={{background:"#F4F6FB",borderRadius:8,padding:"6px 10px",fontSize:11,marginBottom:10,color:"#64748b"}}>
            Win: <b style={{color:"#15803d"}}>{m.result.win}</b> · POTM: <b style={{color:"#B8860B"}}>{m.result.motm}</b>
          </div>}
          {m.perfs.length>0
            ?<div>
              <p style={{color:"#1D428A",fontSize:11,fontWeight:700,margin:"0 0 6px",textTransform:"uppercase",letterSpacing:.5}}>🎯 Perfect Picks</p>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {m.perfs.map(p=>(
                  <div key={p.email} style={{display:"flex",alignItems:"center",gap:6,background:"#EBF0FA",borderRadius:20,padding:"4px 10px"}}>
                    <Av name={p.name} sz={20}/>
                    <span style={{color:"#1D428A",fontSize:12,fontWeight:600}}>{p.name}</span>
                  </div>
                ))}
              </div>
            </div>
            :<p style={{color:"#94a3b8",fontSize:12,margin:0}}>No perfect picks this match</p>
          }
        </div>
      ))}
    </div>}
    ...
    {sc==="adm"&&isAdmin&&<div style={{padding:"16px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {maintenance&&<span style={{background:"#fee2e2",color:"#991b1b",fontSize:10,padding:"3px 8px",borderRadius:12,fontWeight:700}}>🔒 Maintenance</span>}
          <span style={{background:"#dcfce7",color:"#166534",fontSize:11,padding:"3px 10px",borderRadius:20,fontWeight:600}}>Active</span>
        </div>
      </div>
      <div style={{display:"flex",background:"#f1f5f9",borderRadius:10,marginBottom:14,overflow:"hidden",border:"1px solid #e2e8f0",flexWrap:"wrap"}}>
        {[["results","📊 Results"],["users","👥 Users"],["matches","➕ Matches"],["analytics","📈 Stats"],["controls","🎛️ Controls"],["broadcast","📢 Broadcast"]].map(([t,l])=>(
          <button key={t} className={"at"+(admTab===t?" on":"")} onClick={()=>setAdmTab(t)} style={{minWidth:"33%",fontSize:9}}>{l}</button>
        ))}
      </div>
      ...
      {admTab==="results"&&<>
        <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:"10px 14px",marginBottom:14}}>
          <p style={{color:"#15803d",fontSize:12,fontWeight:600,margin:0}}>Enter results below. Saving triggers the group chat notification automatically.</p>
        </div>
        <p className="st">PENDING RESULTS ({ms.filter(m=>!m.result&&!isTBD(m)).length})</p>
        {ms.filter(m=>!m.result&&!isTBD(m)).length===0&&<p style={{color:"#94a3b8",fontSize:12,textAlign:"center",padding:"20px 0"}}>All matches have results entered. ✅</p>}
        {ms.filter(m=>!m.result&&!isTBD(m)).map(m=>{
          const rf=admResultForm[m.id]||{};
          return <div key={m.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 14px",marginBottom:10}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <div style={{display:"flex",gap:8}}>
                <div style={{flex:1}}>
                  <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Toss</p>
                  <select className="sel" value={rf.toss||""} onChange={e=>setAdmResultForm(f=>({...f,[m.id]:{...rf,toss:e.target.value}}))}>
                    <option value="">—</option><option value={m.home}>{m.home}</option><option value={m.away}>{m.away}</option>
                  </select>
                </div>
                <div style={{flex:1}}>
                  <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Winner</p>
                  <select className="sel" value={rf.win||""} onChange={e=>setAdmResultForm(f=>({...f,[m.id]:{...rf,win:e.target.value}}))}>
                    <option value="">—</option><option value={m.home}>{m.home}</option><option value={m.away}>{m.away}</option>
                  </select>
                </div>
              </div>
              <div>
                <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Player of the Match</p>
                <input className="inp" value={rf.motm||""} onChange={e=>setAdmResultForm(f=>({...f,[m.id]:{...rf,motm:e.target.value}}))} placeholder="Full player name" style={{fontSize:13}}/>
              </div>
        ...
        {[...done].reverse().map(m=>(
          <div key={m.id} style={{background:"#f8faff",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
            <TLogo t={m.home} sz={22}/><span style={{color:"#94a3b8",fontSize:11}}>vs</span><TLogo t={m.away} sz={22}/>
            <div style={{flex:1}}>
              <p style={{color:"#1a2540",fontSize:12,fontWeight:600,margin:0}}>{m.mn} · {m.date}</p>
              <p style={{color:"#64748b",fontSize:11,margin:"2px 0 0"}}>W: {m.result.win} · POTM: {m.result.motm}</p>
            </div>
          </div>
        ))}
      </>}
      ...
      {admTab==="users"&&<>
        ...
        {Object.values(users)
          ...
          .sort((a,b)=>(lbScores[b.email]?.pts||0)-(lbScores[a.email]?.pts||0))
          .map(u=>{
            const st=lbScores[u.email]||{pts:0,acc:0,userSp:"",userT4:[]};
            const up=allPicks[ek(u.email)]||{};
            const ex2=exU===u.email;
            const adj=getManualAdj(u.email);
            const mOv=getMatchOverride(u.email);
              ...
              {ex2&&<div style={{padding:"0 14px 14px",borderTop:"1px solid #f1f5f9"}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  {[-50,-25,-10,10,25,50].map(d=>(
                    <button key={d} onClick={()=>adjustPts(u.email,d)} style={{flex:1,padding:"7px 4px",borderRadius:8,background:d>0?"#f0fdf4":"#fef2f2",color:d>0?"#15803d":"#dc2626",border:"1px solid "+(d>0?"#bbf7d0":"#fecaca"),cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:12}}>{d>0?"+":""}{d}</button>
                  ))}
                </div>
                ...
                {ms.filter(m=>up[m.id]||up[String(m.id)]).map(m=>{
                  const emk=ek(u.email);
                  const mOvM=((matchPtsOverride[emk]||{})[m.id])??((matchPtsOverride[emk]||{})[String(m.id)])??0;
                  return <div key={m.id} style={{background:"#f8faff",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                      ...
                      <span style={{color:"#64748b",fontSize:11,flex:1}}>{m.mn}</span>
                      {mOvM!==0&&<span style={{color:"#FF822A",fontSize:11,fontWeight:700}}>{mOvM>0?"+":""}{mOvM} pts</span>}
                    </div>
                    <div style={{display:"flex",gap:6}}>
                      {[-25,-10,-5,5,10,25].map(d=>(
                        <button key={d} onClick={()=>setMatchPts(u.email,m.id,d)} style={{flex:1,padding:"5px 2px",borderRadius:6,background:d>0?"#f0fdf4":"#fef2f2",color:d>0?"#15803d":"#dc2626",border:"1px solid "+(d>0?"#bbf7d0":"#fecaca"),cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11}}>{d>0?"+":""}{d}</button>
                      ))}
                    </div>
                  </div>;
                })}
      ...
      {admTab==="matches"&&<>
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div>
              <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Match Label</p>
              <input className="inp" value={manMatchForm.mn} onChange={e=>setManMatchForm(f=>({...f,mn:e.target.value}))} placeholder="e.g. M75" style={{fontSize:13}}/>
            </div>
            <div style={{display:"flex",gap:8}}>
              <div style={{flex:1}}>
                <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Home</p>
                <select className="sel" value={manMatchForm.home} onChange={e=>setManMatchForm(f=>({...f,home:e.target.value}))}>{TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select>
              </div>
              <div style={{flex:1}}>
                <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Away</p>
                <select className="sel" value={manMatchForm.away} onChange={e=>setManMatchForm(f=>({...f,away:e.target.value}))}>{TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select>
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <div style={{flex:1}}>
                <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Date</p>
                <input className="inp" type="date" value={manMatchForm.date} onChange={e=>setManMatchForm(f=>({...f,date:e.target.value}))} style={{fontSize:13}}/>
              </div>
              <div style={{flex:1}}>
                <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Time IST</p>
                <input className="inp" value={manMatchForm.time} onChange={e=>setManMatchForm(f=>({...f,time:e.target.value}))} placeholder="19:30" style={{fontSize:13}}/>
              </div>
            </div>
            <div>
              <p style={{color:"#64748b",fontSize:10,fontWeight:600,margin:"0 0 4px",textTransform:"uppercase"}}>Venue</p>
      ...
      {admTab==="analytics"&&<>
        {(()=>{
          const mwp=ms.filter(m=>Object.values(allPicks).some(u=>u[m.id]||u[String(m.id)]));
          if(!mwp.length)return <div style={{textAlign:"center",padding:"40px 16px"}}><p style={{fontSize:36}}>📊</p><p style={{color:"#94a3b8",marginTop:12}}>No picks yet</p></div>;
          return mwp.map(m=>{
            const allArr=Object.values(allPicks);
            const tot=allArr.filter(u=>u[m.id]||u[String(m.id)]).length;
            const sp=tot?{tot,tA:allArr.filter(u=>(u[m.id]??u[String(m.id)])?.toss===m.home).length,tB:tot-allArr.filter(u=>(u[m.id]??u[String(m.id)])?.toss===m.home).length,wA:allArr.filter(u=>(u[m.id]??u[String(m.id)])?.win===m.home).length,wB:tot-allArr.filter(u=>(u[m.id]??u[String(m.id)])?.win===m.home).length}:null;
            const io=anM===m.id;
            ...
            return <div key={m.id} className="ac" style={{cursor:"pointer"}} onClick={()=>setAnM(io?null:m.id)}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  <p className="C" style={{color:"#1a2540",fontSize:14,fontWeight:700,margin:0}}>{m.home} vs {m.away}</p>
                  <p style={{color:"#64748b",fontSize:11,margin:0}}>{sp?.tot||0} picks · {m.mn}</p>
                </div>
      ...
      {admTab==="controls"&&<div>
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          <div className="ctrl-row">
            <div><p style={{color:"#1a2540",fontSize:13,fontWeight:600,margin:0}}>🔧 Maintenance Mode</p><p style={{color:"#94a3b8",fontSize:11,margin:"2px 0 0"}}>Locks app for non-admins</p></div>
            <Toggle on={maintenance} onChange={toggleMaintenance}/>
          </div>
          <div className="ctrl-row">
            <div><p style={{color:"#1a2540",fontSize:13,fontWeight:600,margin:0}}>📤 Export Leaderboard CSV</p></div>
            <button onClick={exportCSV} style={{padding:"7px 14px",borderRadius:8,background:"#EBF0FA",color:"#1D428A",border:"1px solid #bfdbfe",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:12}}>Export</button>
          </div>
          <div className="ctrl-row">
            <div><p style={{color:"#1a2540",fontSize:13,fontWeight:600,margin:0}}>🔇 Mute All Chat</p></div>
            <Toggle on={!!chatMuted} onChange={async v=>{setChatMuted(v);await DB.set("chatmuted",v);toast2(v?"Chat muted for everyone":"Chat reopened");}}/>
          </div>
        </div>
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          {ms.filter(m=>!m.result&&!isTBD(m)).slice(0,8).map(m=>{
            const isDouble=doubleMatch!=null&&Number(doubleMatch)===Number(m.id);
            return <div key={m.id} className="ctrl-row" style={{padding:"10px 0"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,flex:1,minWidth:0}}>
                <TLogo t={m.home} sz={16}/><span className="C" style={{fontSize:11,fontWeight:700}}>{m.home}</span>
                <span style={{color:"#94a3b8",fontSize:10}}>vs</span>
                <TLogo t={m.away} sz={16}/><span className="C" style={{fontSize:11,fontWeight:700}}>{m.away}</span>
                <span style={{color:"#94a3b8",fontSize:10,marginLeft:4}}>{m.mn}</span>
              </div>
              <button onClick={async()=>{const v=isDouble?null:m.id;setDoubleMatch(v);await DB.set("doublematch",v);toast2(v?"⚡ 2× set for "+m.mn:"2× removed");}} style={{padding:"5px 10px",borderRadius:8,background:isDouble?"linear-gradient(135deg,#FF822A,#D4AF37)":"#f8faff",color:isDouble?"#fff":"#64748b",border:"1px solid "+(isDouble?"#FF822A":"#e2e8f0"),cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11,flexShrink:0}}>{isDouble?"2× ON":"Set 2×"}</button>
            </div>;
          })}
        </div>
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:10}}>
            {TEAMS.map(t=>(
              <button key={t} onClick={()=>setSeasonWinner(t)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",borderRadius:10,background:sw===t?"#1D428A":"#f8faff",border:"1.5px solid "+(sw===t?"#1D428A":"#e2e8f0"),cursor:"pointer"}}>
                <TLogo t={t} sz={20}/><span className="C" style={{fontSize:12,fontWeight:700,color:sw===t?"#fff":"#475569"}}>{t}</span>{sw===t&&<span style={{fontSize:10}}>🏆</span>}
              </button>
            ))}
          </div>
        ...
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          <p style={{color:"#94a3b8",fontSize:11,marginBottom:10}}>Tap to cycle: Auto → Force Lock → Force Unlock → Auto</p>
          {ms.filter(m=>!m.result&&!isTBD(m)).map(m=>{
            const lstate=lockedMatches[m.id]??lockedMatches[String(m.id)];
            return <div key={m.id} className="ctrl-row">
              <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          {Object.values(users).filter(u=>u.email!==email).map(u=>{
            const isMuted=(mutedUsers||{})[ek(u.email)];
            return <div key={u.email} className="ctrl-row">
              <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0}}>
                <Av name={u.name} sz={24}/>
                <p style={{color:"#1a2540",fontSize:12,fontWeight:600,margin:0}}>{u.name}</p>
              </div>
        ...
        <div style={{background:"#fff",border:"1px solid #fecaca",borderRadius:12,padding:"14px",marginBottom:14}}>
          ...
          <div className="ctrl-row">
            <div>
              <p style={{color:"#1a2540",fontSize:13,fontWeight:600,margin:0}}>🗑️ Reset Season</p>
              <p style={{color:"#94a3b8",fontSize:11,margin:"2px 0 0"}}>Wipes all picks, results, pts — irreversible.</p>
            </div>
            <button onClick={async()=>{
              if(!confirm("RESET entire season? This CANNOT be undone."))return;
              await Promise.all([DB.set("ap",{}),DB.set("rm",{}),DB.set("sp",{}),DB.set("t4",{}),DB.set("ptsadj",{}),DB.set("doublematch",null),DB.set("sw",null),DB.set("matchptsoverride",{}),DB.set("manmatches",[]),DB.set("bracket",null),DB.set("lockedm",{})]);
              setAllPicks({});setMyPicks({});setMs(buildBaseMatches());setManualPtsAdj({});setSw(null);setDoubleMatch(null);setMatchPtsOverride({});setBracket(null);setLockedMatches({});setSpk({});setT4pk({});setMySp("");setMyT4([]);
              toast2("Season reset","ok");
      ...
      {admTab==="broadcast"&&<>
        <div className="ac">
          ...
          {pinnedBc&&<div style={{marginTop:12,background:"#1D428A",borderRadius:8,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{color:"#fff",fontSize:12,fontWeight:600}}>📌 {pinnedBc}</span>
            <button onClick={clearPin} style={{background:"none",border:"none",color:"#bfdbfe",cursor:"pointer",fontSize:12,fontWeight:600}}>Clear</button>
          </div>}
        </div>
        ...
        {[...bc].reverse().map(b=>(
          <div key={b.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1}}>
              <p style={{color:"#1a2540",fontSize:13,margin:"0 0 3px"}}>{b.msg}</p>
              <p style={{color:"#94a3b8",fontSize:11,margin:0}}>{new Date(b.ts).toLocaleString("en-IN",{timeZone:"Asia/Kolkata",day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit",hour12:true})}</p>
            </div>
