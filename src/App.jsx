...
try{localStorage.removeItem("ipl26_session");}catch(e){}if(!cancelled)setSc("login");return;}
  ...
  async function approveUser(emk){const pu=await DB.get("pending")||{};const entry=pu[emk];if(!entry)return;delete pu[emk];const u2=await DB.get("u")||{};u2[entry.email]={...entry,approved:true};await DB.set("u",u2);await DB.set("pending",pu);setUsers({...users,[entry.email]:{...entry,approved:true}});setPendingUsers({...pu});toast2(entry.name+" approved! ✅","ok");const latest=await DB.get("ch")||[];const nc=capChat([...latest,{id:Date.now(),email:"__sys__",name:"Fantasy Bot",text:"🎉 "+entry.name+" has joined! Welcome!",ts:Date.now(),sys:true}]);setChat(nc);await DB.set("ch",nc);}
...
{(()=>{
            ...
            return<div style={{borderTop:"1px solid #f1f5f9",paddingTop:6,marginTop:4}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}>
                <p style={{fontSize:9,color:"#94a3b8",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:0}}>🎲 Season Prop Bets · {propAnsweredCount}/{PROP_QUESTIONS.length} answered</p>
    ...
    {sc==="rules"&&<div style={{padding:"16px"}}>
      ...
      {[
        ...
        ["🎲 Season Prop Bets",`${PROP_QUESTIONS.length} compulsory season-long questions answered once during onboarding — all must be answered before you can start playing:\n${PROP_QUESTIONS.map(q=>"• "+q.label+": +"+PTS.prop+"pts").join("\n")}\nAdmin enters correct answers at season end. Total potential: +${PTS.prop*PROP_QUESTIONS.length}pts.`],
