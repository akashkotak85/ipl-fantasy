  ...
  return(
    <div className="ah-wrap">
      ...
      <div className="ah-body">
        ...
        {/* ── CRICKET ADMIN TAB ───────────────────────────────── */}
        {!loading&&tab==="cricket"&&<>
          {statuses.ipl2026==="finished"&&(
            <div style={{background:"#f5f3ff",border:"1px solid #c4b5fd",borderRadius:12,
              padding:"12px 14px",marginBottom:12,fontSize:12,color:"#7c3aed",fontWeight:600}}>
              🏁 IPL 2026 is closed. Changes here will not affect the frozen leaderboard.
            </div>
          )}

          {/* Result entry */}
          <div className="ah-card">
            <Section title="📋 Enter Match Result" accent="#1D428A">
              <select className="ah-select" value={cSelId}
                onChange={e=>onSelectCricketMatch(e.target.value)} style={{marginBottom:10}}>
                <option value="">Select match…</option>
                {cMs.map(m=>{
                  const done=!!(cRm[m.id]||cRm[String(m.id)]);
                  return<option key={m.id} value={m.id}>
                    {m.mn}: {m.home} vs {m.away}{done?" ✅":""}</option>;
                })}
              </select>
              ...
              {cSelMatch&&<>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                  <div>
                    <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Winner</p>
                    <select className="ah-select" value={cDraft.win}
                      onChange={e=>setCDraft(d=>({...d,win:e.target.value}))}>
                      <option value="">Select winner…</option>
                      {[cSelMatch.home,cSelMatch.away].map(t=>(
                        <option key={t} value={t}>{t}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>POTM (last name)</p>
                    <input className="ah-input" value={cDraft.motm}
                      onChange={e=>setCDraft(d=>({...d,motm:e.target.value}))}
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                  <div>
                    <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>1st Innings Score Band</p>
                    <select className="ah-select" value={cDraft.sb}
                      onChange={e=>setCDraft(d=>({...d,sb:e.target.value}))}>
                  ...
                  {BONUS_QUESTIONS[Number(cSelId)]&&<div>
                    <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Bonus Answer</p>
                    <select className="ah-select" value={cDraft.bonus===null?"":String(cDraft.bonus)}
                      onChange={e=>setCDraft(d=>({...d,bonus:e.target.value===""?null:e.target.value==="true"}))}>
                ...
                {BONUS_QUESTIONS[Number(cSelId)]&&<p style={{fontSize:11,color:"#94a3b8",
                  fontStyle:"italic",margin:"0 0 8px",background:"#f8faff",borderRadius:8,padding:"8px 10px"}}>
                  ❓ {BONUS_QUESTIONS[Number(cSelId)]}
                </p>}

                <div style={{display:"flex",gap:8}}>
                  <button className="ah-btn ah-btn-primary" style={{flex:1}}
                    disabled={!cDraft.win||cSaving} onClick={saveCricketResult}>
                    {cSaving?"Saving…":"💾 Save Result"}
                  </button>
                  {(cRm[Number(cSelId)]||cRm[String(cSelId)])&&(
          ...
          {/* Match lock controls */}
          <div className="ah-card">
            <Section title="🔒 Match Lock Controls" accent="#1D428A">
              <p style={{fontSize:11,color:"#94a3b8",margin:"0 0 10px"}}>
                Locked → no picks. Unlocked → overrides countdown. Auto → uses 35-min countdown.
              </p>
              <div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:320,overflowY:"auto"}}>
                {cMs.filter(m=>!m.result).map(m=>{
                  const st=cLocked[m.id]??cLocked[String(m.id)];
                  return(
                    <div key={m.id} style={{display:"flex",alignItems:"center",gap:8,
                      padding:"8px 10px",background:"#f8faff",borderRadius:8}}>
                      <span style={{fontSize:11,fontWeight:700,color:"#64748b",minWidth:24}}>{m.mn}</span>
                      <span style={{flex:1,fontSize:12,color:"#0a1628"}}>{m.home} vs {m.away}</span>
                      <Badge
                        label={st==="locked"?"🔒":st==="unlocked"?"🔓":"⏱️"}
                        color={st==="locked"?"#991b1b":st==="unlocked"?"#15803d":"#d97706"}
                        bg={st==="locked"?"#fef2f2":st==="unlocked"?"#f0fdf4":"#fffbeb"}
                      />
                      <button className="ah-btn ah-btn-sm" style={{background:"#e8edf5",color:"#475569",border:"none"}}
                        onClick={()=>toggleCricketLock(m.id)}>Toggle</button>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Double header */}
          <div className="ah-card">
            <Section title="⚡ Double Header Match" accent="#1D428A">
              <p style={{fontSize:11,color:"#94a3b8",margin:"0 0 10px"}}>
                Selected match earns 2× points on all categories.
              </p>
              <select className="ah-select" value={cDbl??""} onChange={e=>setCricketDouble(e.target.value)}>
                <option value="">None — no double header</option>
                {cMs.map(m=><option key={m.id} value={m.id}>{m.mn}: {m.home} vs {m.away}</option>)}
              </select>
            </Section>
          </div>
        </>}
        ...
        {/* ── FIFA ADMIN TAB ───────────────────────────────────── */}
        {!loading&&tab==="fifa"&&<>
          {statuses.fifa2026==="finished"&&(
            <div style={{background:"#f5f3ff",border:"1px solid #c4b5fd",borderRadius:12,
              padding:"12px 14px",marginBottom:12,fontSize:12,color:"#7c3aed",fontWeight:600}}>
              🏁 FIFA 2026 is closed. Changes here will not affect the frozen leaderboard.
            </div>
          )}

          {/* Result entry */}
          <div className="ah-card">
            <Section title="📋 Enter Match Result" accent="#004B87">
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div>
                  <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Match ID</p>
                  <input className="ah-input" type="number" value={fSelId}
                    onChange={e=>{setFSelId(e.target.value);
                      const ex=fRm[Number(e.target.value)];
                      setFDraft({win:ex?.win||"",motm:ex?.motm||"",score:ex?.score||""});
                    }} placeholder="e.g. 1"/>
                </div>
                <div>
                  <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Winner</p>
                  <select className="ah-select" value={fDraft.win}
                    onChange={e=>setFDraft(d=>({...d,win:e.target.value}))}>
                    <option value="">Select winner…</option>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                <div>
                  <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Man of the Match</p>
                  <input className="ah-input" value={fDraft.motm}
                    onChange={e=>setFDraft(d=>({...d,motm:e.target.value}))} placeholder="Player surname"/>
                </div>
                <div>
                  <p style={{fontSize:11,color:"#64748b",margin:"0 0 4px",fontWeight:600}}>Score</p>
                  <input className="ah-input" value={fDraft.score}
                    onChange={e=>setFDraft(d=>({...d,score:e.target.value}))} placeholder="e.g. 2–1"/>
                </div>
              </div>

              <div style={{display:"flex",gap:8}}>
                <button className="ah-btn ah-btn-primary" style={{flex:1,background:"#004B87"}}
                  disabled={!fSelId||!fDraft.win||fSaving} onClick={saveFifaResult}>
          ...
          {/* Recent results */}
          {Object.keys(fRm).length>0&&<div className="ah-card">
            <Section title="✅ Results Entered" accent="#004B87">
              <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:240,overflowY:"auto"}}>
                {Object.entries(fRm).sort((a,b)=>Number(a[0])-Number(b[0])).map(([id,r])=>(
                  <div key={id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",
                    background:"#f8faff",borderRadius:8,fontSize:12}}>
                    <span style={{color:"#94a3b8",fontWeight:700,minWidth:30}}>M{id}</span>
                    <span style={{flex:1,fontWeight:600,color:"#0a1628"}}>
                      {r.win}{r.score?` (${r.score})`:""}
                    </span>
                    <span style={{color:"#64748b"}}>{r.motm&&!isNR(r.motm)?r.motm:""}</span>
                  </div>
                ))}
              </div>
            </Section>
          ...
          {/* Season awards */}
          <div className="ah-card">
            <Section title="🏆 Season Awards (Actual)" accent="#004B87">
              <p style={{fontSize:11,color:"#94a3b8",margin:"0 0 10px"}}>
                Set these when the tournament is over to award season points.
              </p>

              <p style={{fontSize:12,fontWeight:700,color:"#0a1628",margin:"0 0 6px"}}>🏆 Champion</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
                {FIFA_TEAMS.map(t=>(
                  <button key={t} onClick={()=>saveFifaSw(t)} style={{padding:"5px 10px",borderRadius:8,
                    background:fSw===t?"#004B87":"#f1f5f9",color:fSw===t?"#fff":"#475569",
                    border:"none",cursor:"pointer",fontSize:11,fontWeight:fSw===t?700:400}}>
                    {t}{fSw===t?" ✅":""}
                  </button>
                ))}
              </div>

              <p style={{fontSize:12,fontWeight:700,color:"#0a1628",margin:"0 0 6px"}}>
                🏅 Top 4 · {fAt4.length}/4 set
              </p>
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {FIFA_TEAMS.map(t=>{
                  const sel=fAt4.includes(t);
                  return(
                    <button key={t} onClick={()=>toggleFifaTop4(t)} style={{padding:"5px 10px",
                      borderRadius:8,background:sel?"#004B87":"#f1f5f9",
                      color:sel?"#fff":"#475569",border:"none",cursor:"pointer",
          ...
          {/* FIFA match locks */}
          <div className="ah-card">
            <Section title="🔒 Match Lock Controls" accent="#004B87">
              <p style={{fontSize:11,color:"#94a3b8",margin:"0 0 10px"}}>
                Enter a match ID to toggle its lock state.
              </p>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input className="ah-input" type="number" placeholder="Match ID"
                  id="fLockId" style={{maxWidth:120}}/>
                <button className="ah-btn ah-btn-primary" style={{background:"#004B87"}}
                  onClick={()=>{
                    const id=Number(document.getElementById("fLockId").value);
                    if(!id) return;
                    toggleFifaLock(id);
                  }}>Toggle Lock</button>
              </div>
              {Object.keys(fLocked).length>0&&<div style={{marginTop:10,display:"flex",flexDirection:"column",gap:4}}>
                {Object.entries(fLocked).map(([id,st])=>(
                  <div key={id} style={{display:"flex",alignItems:"center",gap:8,
                    padding:"6px 10px",background:"#f8faff",borderRadius:8,fontSize:12}}>
                    <span style={{fontWeight:700,color:"#64748b",minWidth:30}}>M{id}</span>
                    <Badge label={st==="locked"?"🔒 Locked":"🔓 Unlocked"}
                      color={st==="locked"?"#991b1b":"#15803d"}
                      bg={st==="locked"?"#fef2f2":"#f0fdf4"}/>
                  </div>
                ))}
              </div>}
            </Section>
